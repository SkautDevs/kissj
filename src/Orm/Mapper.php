...
class Mapper implements IMapper
{
    ...
    public function getTableByRepositoryClass(string $repositoryClass): string
    {
        ...
        if ($match === false || isset($matches[1]) === false) {
    ...
    {
        $pregReplaced = preg_replace_callback('#(?<=.)([A-Z])#', fn (array $m): string => '_' . strtolower($m[1]), $string);
    ...
    {
        $pregReplaced = preg_replace_callback('#_(.)#', fn (array $m): string => strtoupper($m[1]), $string);
        if ($pregReplaced === null) {
            throw new \RuntimeException('preg_replace_callback failed');
        }
