...
class OwnerTransferSelfTest extends AppTestCase
{
    ...
    public function testShowTransferTicketRejectsOwnTieCode(): void
    {
        $app = $this->getTestApp();
        $event = $this->getOwnerTransferEvent($app);
        $user = $this->makePaidParticipant($app, $event);
        ...
        )->withQueryParams(['tieCode' => $ownTieCode]));

        self::assertSame(200, $response->getStatusCode());
        $body = (string) $response->getBody();
        $translator = $this->getService($app, TranslatorInterface::class);
        self::assertStringContainsString($translator->trans('flash.warning.cannotTransferToYourself'), $body);
        // isTransferToSelf() nulls both $to and $tieCode, so the recipient block must not render
        self::assertStringNotContainsString($translator->trans('ticketTransfer.recipient'), $body);
    }
