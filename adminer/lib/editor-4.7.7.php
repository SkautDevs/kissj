...
idf_unescape($u) {
    $Bd = substr($u, -1);
    return
        str_replace($Bd.$Bd, $Bd, substr($u, 1, -1));
}
...
function
remove_slashes($Se, $qc = false) {
    if (get_magic_quotes_gpc()) {
        while (list($y, $X) = each($Se)) {
            foreach ($X
                     as $sd => $W) {
                unset($Se[$y][$sd]);
                if (is_array($W)) {
                    $Se[$y][stripslashes($sd)] = $W;
                    $Se[] =& $Se[$y][stripslashes($sd)];
                } else$Se[$y][stripslashes($sd)] = ($qc ? $W : stripslashes($W));
            }
        }
    }
}
...
function
bracket_escape($u, $Ia = false) {
    static $vg = [':' => ':1', ']' => ':2', '[' => ':3', '"' => ':4'];
    return
        strtr($u, ($Ia ? array_flip($vg) : $vg));
}

function
min_version($Wg, $Nd = "", $i = null) {
    global $h;
    if (!$i) $i = $h;
    $Bf = $i->server_info;
    if ($Nd && preg_match('~([\d.]+)-MariaDB~', $Bf, $A)) {
        $Bf = $A[1];
        $Wg = $Nd;
    }
    return (version_compare($Bf, $Wg) >= 0);
}
...
function
script($Kf, $ug = "\n") {
    return "<script".nonce().">$Kf</script>$ug";
}
...
function
checkbox($B, $Y, $Xa, $yd = "", $pe = "", $bb = "", $zd = "") {
    $J = "<input type='checkbox' name='$B' value='".h($Y)."'".($Xa ? " checked" : "").($zd ? " aria-labelledby='$zd'" : "").">".($pe ? script("qsl('input').onclick = function () { $pe };", "") : "");
    return ($yd != "" || $bb ? "<label".($bb ? " class='$bb'" : "").">$J".h($yd)."</label>" : $J);
}

function
optionlist($C, $wf = null, $Sg = false) {
    $J = "";
    foreach ($C
             as $sd => $W) {
        $ue = [$sd => $W];
        if (is_array($W)) {
            $J .= '<optgroup label="'.h($sd).'">';
            $ue = $W;
        }
        foreach ($ue
                 as $y => $X) $J .= '<option'.($Sg || is_string($y) ? ' value="'.h($y).'"' : '').(($Sg || is_string($y) ? (string)$y : $X) === $wf ? ' selected' : '').'>'.h($X);
...
function
html_select($B, $C, $Y = "", $oe = true, $zd = "") {
    if ($oe) return "<select name='".h($B)."'".($zd ? " aria-labelledby='$zd'" : "").">".optionlist($C, $Y)."</select>".(is_string($oe) ? script("qsl('select').onchange = function () { $oe };", "") : "");
    $J = "";
    foreach ($C
             as $y => $X) $J .= "<label><input type='radio' name='".h($B)."' value='".h($y)."'".($y == $Y ? " checked" : "").">".h($X)."</label>";
    return $J;
}

function
select_input($Da, $C, $Y = "", $oe = "", $Je = "") {
    $dg = ($C ? "select" : "input");
    return "<$dg$Da".($C ? "><option value=''>$Je".optionlist($C, $Y, true)."</select>" : " size='10' value='".h($Y)."' placeholder='$Je'>").($oe ? script("qsl('$dg').onchange = $oe;", "") : "");
}

function
confirm($Vd = "", $xf = "qsl('input')") {
    return
        script("$xf.onclick = function () { return confirm('".($Vd ? js_escape($Vd) : lang(0))."'); };", "");
}

function
print_fieldset($t, $Dd, $Zg = false) {
    echo "<fieldset><legend>", "<a href='#fieldset-$t'>$Dd</a>", script("qsl('a').onclick = partial(toggle, 'fieldset-$t');", ""), "</legend>", "<div id='fieldset-$t'".($Zg ? "" : " class='hidden'").">\n";
}

function
bold($Qa, $bb = "") {
    return ($Qa ? " class='active $bb'" : ($bb ? " class='$bb'" : ""));
}
...
function
set_password($Vg, $O, $V, $F) {
    $_SESSION["pwds"][$Vg][$O][$V] = ($_COOKIE["adminer_key"] && is_string($F) ? [encrypt_string($F, $_COOKIE["adminer_key"])] : $F);
}
...
get_vals($G, $e = 0) {
    ...
    $I = $h->query($G);
    if (is_object($I)) {
        while ($K = $I->fetch_row()) $J[] = $K[$e];
    }
    return $J;
}

function
get_key_vals($G, $i = null, $Ef = true) {
    global $h;
    if (!is_object($i)) $i = $h;
    $J = [];
    $I = $i->query($G);
...
get_rows($G, $i = null, $o = "<p class='error'>") {
    ...
    } elseif (!$I && !is_object($i) && $o && defined("PAGE_HEADER")) echo $o.error()."\n";
    return $J;
}
...
where($Z, $q = []) {
    ...
    foreach ((array)$Z["where"] as $y => $X) {
        ...
        $e = escape_key($y);
        $J[] = $e.($x == "sql" && is_numeric($X) && preg_match('~\.~', $X) ? " LIKE ".q($X) : ($x == "mssql" ? " LIKE ".q(preg_replace('~[_%[]~', '[\0]', $X)) : " = ".unconvert_field($q[$y], q($X))));
        if ($x == "sql" && preg_match('~char|text~', $q[$y]["type"]) && preg_match("~[^ -@]~", $X)) $J[] = "$e = ".q($X)." COLLATE ".charset($h)."_bin";
    }
...
function
where_link($s, $e, $Y, $re = "=") {
    return "&where%5B$s%5D%5Bcol%5D=".urlencode($e)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y !== null ? $re : "IS NULL"))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);
}

function
convert_fields($f, $q, $M = []) {
    $J = "";
    foreach ($f
             as $y => $X) {
        if ($M && !in_array(idf_escape($y), $M)) continue;
        $_a = convert_field($q[$y]);
        if ($_a) $J .= ", $_a AS ".idf_escape($y);
    }
    return $J;
}

function
cookie($B, $Y, $Gd = 2592000) {
    global $aa;
    return
        header("Set-Cookie: $B=".urlencode($Y).($Gd ? "; expires=".gmdate("D, d M Y H:i:s", time() + $Gd)." GMT" : "")."; path=".preg_replace('~\?.*~', '', $_SERVER["REQUEST_URI"]).($aa ? "; secure" : "")."; HttpOnly; SameSite=lax", false);
}
...
function
stop_session($wc = false) {
    $Rg = ini_bool("session.use_cookies");
    if (!$Rg || $wc) {
        session_write_close();
        if ($Rg && @ini_set("session.use_cookies", false) === false) session_start();
    }
}
...
function
auth_url($Vg, $O, $V, $m = null) {
    global $Jb;
    preg_match('~([^?]*)\??(.*)~', remove_from_uri(implode("|", array_keys($Jb))."|username|".($m !== null ? "db|" : "").session_name()), $A);
    return "$A[1]?".(sid() ? SID."&" : "").($Vg != "server" || $O != "" ? urlencode($Vg)."=".urlencode($O)."&" : "")."username=".urlencode($V).($m != "" ? "&db=".urlencode($m) : "").($A[2] ? "&$A[2]" : "");
}
...
redirect($Id, $Vd = null) {
    if ($Vd !== null) {
        restart_session();
        $_SESSION["messages"][preg_replace('~^[^?]*~', '', ($Id !== null ? $Id : $_SERVER["REQUEST_URI"]))][] = $Vd;
    }
    if ($Id !== null) {
        if ($Id == "") $Id = ".";
        header("Location: $Id");
...
function
query_redirect($G, $Id, $Vd, $cf = true, $cc = true, $jc = false, $kg = "") {
    global $h, $o, $b;
    if ($cc) {
        $Qf = microtime(true);
        $jc = !$h->query($G);
    ...
    $Nf = "";
    if ($G) $Nf = $b->messageQuery($G, $kg, $jc);
    if ($jc) {
        $o = error().$Nf.script("messagesPrint();");
        return
            false;
    }
    if ($cf) redirect($Id, $Vd.$Nf);
    return
        true;
}
...
queries($G) {
    ...
    static $We = [];
    static $Qf;
    if (!$Qf) $Qf = microtime(true);
    if ($G === null) return
        [implode("\n", $We), format_time($Qf)];
    $We[] = (preg_match('~;$~', $G) ? "DELIMITER ;;\n$G;\nDELIMITER " : $G).";";
    return $h->query($G);
}

function
apply_queries($G, $T, $Zb = 'table') {
    foreach ($T
             as $R) {
        if (!queries("$G ".$Zb($R))) return
            false;
    }
    return
        true;
...
function
queries_redirect($Id, $Vd, $cf) {
    [$We, $kg] = queries(null);
    return
        query_redirect($We, $Id, $Vd, $cf, false, !$cf, $kg);
}

function
format_time($Qf) {
    return
        lang(1, max(0, microtime(true) - $Qf));
}
...
remove_from_uri($Be = "") {
    return
        substr(preg_replace("~(?<=[?&])($Be".(SID ? "" : "|".session_name()).")=[^&]*&~", '', relative_uri()."&"), 0, -1);
}

function
pagination($D, $wb) {
    return " ".($D == $wb ? $D + 1 : '<a href="'.h(remove_from_uri("page").($D ? "&page=$D".($_GET["next"] ? "&next=".urlencode($_GET["next"]) : "") : "")).'">'.($D + 1)."</a>");
}
...
get_file($y, $_b = false) {
    ...
    foreach ($oc
             as $y => $X) $oc[$y] = (array)$X;
    $J = '';
    foreach ($oc["error"] as $y => $o) {
        if ($o) return $o;
        $B = $oc["name"][$y];
        $rg = $oc["tmp_name"][$y];
        $pb = file_get_contents($_b && preg_match('~\.gz$~', $B) ? "compress.zlib://$rg" : $rg);
        if ($_b) {
            $Qf = substr($pb, 0, 3);
            if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Qf, $df)) $pb = iconv("utf-16", "utf-8", $pb); elseif ($Qf == "\xEF\xBB\xBF") $pb = substr($pb, 3);
            $J .= $pb."\n\n";
        } else$J .= $pb;
    }
    return $J;
...
function
upload_error($o) {
    $Sd = ($o == UPLOAD_ERR_INI_SIZE ? ini_get("upload_max_filesize") : 0);
    return ($o ? lang(2).($Sd ? " ".lang(3, $Sd) : "") : lang(4));
}

function
repeat_pattern($He, $Ed) {
    return
        str_repeat("$He{0,65535}", $Ed / 65535)."$He{0,".($Ed % 65535)."}";
}
...
column_foreign_keys($R) {
    global $b;
    $J = [];
    foreach ($b->foreignKeys($R) as $_c) {
        foreach ($_c["source"] as $X) $J[$X][] = $_c;
    }
    return $J;
}

function
enum_input($U, $Da, $p, $Y, $Ub = null) {
    global $b;
    preg_match_all("~'((?:[^']|'')*)'~", $p["length"], $Pd);
    $J = ($Ub !== null ? "<label><input type='$U'$Da value='$Ub'".((is_array($Y) ? in_array($Ub, $Y) : $Y === 0) ? " checked" : "")."><i>".lang(7)."</i></label>" : "");
    foreach ($Pd[1] as $s => $X) {
        $X = stripcslashes(str_replace("''", "'", $X));
        $Xa = (is_int($Y) ? $Y == $s + 1 : (is_array($Y) ? in_array($s + 1, $Y) : $Y === $X));
        $J .= " <label><input type='$U'$Da value='".($s + 1)."'".($Xa ? ' checked' : '').'>'.h($b->editVal($X, $p)).'</label>';
    }
    return $J;
}
...
input($p, $Y, $Fc) {
    global $Eg, $b, $x;
    $B = h(bracket_escape($p["field"]));
    echo "<td class='function'>";
    if (is_array($Y) && !$Fc) {
        $ya = [$Y];
        if (version_compare(PHP_VERSION, 5.4) >= 0) $ya[] = JSON_PRETTY_PRINT;
        $Y = call_user_func_array('json_encode', $ya);
        $Fc = "json";
    }
    $if = ($x == "mssql" && $p["auto_increment"]);
    if ($if && !$_POST["save"]) $Fc = null;
    $Gc = (isset($_GET["select"]) || $if ? ["orig" => lang(8)] : []) + $b->editFunctions($p);
    $Da = " name='fields[$B]'";
    if ($p["type"] == "enum") echo
        h($Gc[""])."<td>".$b->editInput($_GET["edit"], $p, $Da, $Y); else {
        $Nc = (in_array($Fc, $Gc) || isset($Gc[$Fc]));
        echo (count($Gc) > 1 ? "<select name='function[$B]'>".optionlist($Gc, $Fc === null || $Nc ? $Fc : "")."</select>".on_help("getTarget(event).value.replace(/^SQL\$/, '')", 1).script("qsl('select').onchange = functionChange;", "") : h(reset($Gc))).'<td>';
        $kd = $b->editInput($_GET["edit"], $p, $Da, $Y);
        if ($kd != "") echo $kd; elseif (preg_match('~bool~', $p["type"])) echo "<input type='hidden'$Da value='0'>"."<input type='checkbox'".(preg_match('~^(1|t|true|y|yes|on)$~i', $Y) ? " checked='checked'" : "")."$Da value='1'>";
		elseif ($p["type"] == "set") {
            preg_match_all("~'((?:[^']|'')*)'~", $p["length"], $Pd);
            foreach ($Pd[1] as $s => $X) {
                $X = stripcslashes(str_replace("''", "'", $X));
                $Xa = (is_int($Y) ? ($Y >> $s) & 1 : in_array($X, explode(",", $Y), true));
                echo " <label><input type='checkbox' name='fields[$B][$s]' value='".(1 << $s)."'".($Xa ? ' checked' : '').">".h($b->editVal($X, $p)).'</label>';
            }
        } elseif (preg_match('~blob|bytea|raw|file~', $p["type"]) && ini_bool("file_uploads")) echo "<input type='file' name='fields-$B'>";
		elseif (($hg = preg_match('~text|lob|memo~i', $p["type"])) || preg_match("~\n~", $Y)) {
            if ($hg && $x != "sqlite") $Da .= " cols='50' rows='12'"; else {
                $L = min(12, substr_count($Y, "\n") + 1);
                $Da .= " cols='30' rows='$L'".($L == 1 ? " style='height: 1.2em;'" : "");
            }
            echo "<textarea$Da>".h($Y).'</textarea>';
        } elseif ($Fc == "json" || preg_match('~^jsonb?$~', $p["type"])) echo "<textarea$Da cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';
        else {
            $Ud = (!preg_match('~int~', $p["type"]) && preg_match('~^(\d+)(,(\d+))?$~', $p["length"], $A) ? ((preg_match("~binary~", $p["type"]) ? 2 : 1) * $A[1] + ($A[3] ? 1 : 0) + ($A[2] && !$p["unsigned"] ? 1 : 0)) : ($Eg[$p["type"]] ? $Eg[$p["type"]] + ($p["unsigned"] ? 0 : 1) : 0));
            if ($x == 'sql' && min_version(5.6) && preg_match('~time~', $p["type"])) $Ud += 7;
            echo "<input".((!$Nc || $Fc === "") && preg_match('~(?<!o)int(?!er)~', $p["type"]) && !preg_match('~\[\]~', $p["full_type"]) ? " type='number'" : "")." value='".h($Y)."'".($Ud ? " data-maxlength='$Ud'" : "").(preg_match('~char|binary~', $p["type"]) && $Ud > 20 ? " size='40'" : "")."$Da>";
        }
        ...
        if ($rc) echo
        script("mixin(qsl('td'), {onchange: partial(skipOriginal, $rc), oninput: function () { this.onchange(); }});");
    }
}
...
process_input($p) {
    ...
    if ($p["auto_increment"] && $Y == "") return
        null;
    if ($Fc == "orig") return (preg_match('~^CURRENT_TIMESTAMP~i', $p["on_update"]) ? idf_escape($p["field"]) : false);
    ...
    if (preg_match('~blob|bytea|raw|file~', $p["type"]) && ini_bool("file_uploads")) {
        $oc = get_file("fields-$u");
        if (!is_string($oc)) return
            false;
        return $n->quoteBinary($oc);
    }
    return $b->processInput($p, $Y, $Fc);
}
...
fields_from_edit() {
    ...
    foreach ((array)$_POST["fields"] as $y => $X) {
        ...
        $J[$B] = ["field" => $B, "privileges" => ["insert" => 1, "update" => 1], "null" => 1, "auto_increment" => ($y == $n->primary),];
    }
    return $J;
}
...
search_tables() {
    ...
    foreach (table_status('', true) as $R => $S) {
        $B = $b->tableName($S);
        if (isset($S["Engine"]) && $B != "" && (!$_POST["tables"] || in_array($R, $_POST["tables"]))) {
            $I = $h->query("SELECT".limit("1 FROM ".table($R), " WHERE ".implode(" AND ", $b->selectSearchProcess(fields($R), [])), 1));
            if (!$I || $I->fetch_row()) {
                $Qe = "<a href='".h(ME."select=".urlencode($R)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$B</a>";
                echo "$zf<li>".($I ? $Qe : "<p class='error'>$Qe: ".error())."\n";
                $zf = "";
            }
        }
    }
    echo ($zf ? "<p class='message'>".lang(9) : "</ul>")."\n";
}

function
dump_headers($Wc, $ae = false) {
    global $b;
    $J = $b->dumpHeaders($Wc, $ae);
    $ze = $_POST["output"];
    if ($ze != "text") header("Content-Disposition: attachment; filename=".$b->dumpFilename($Wc).".$J".($ze != "file" && !preg_match('~[^0-9a-z]~', $ze) ? ".$ze" : ""));
    session_write_close();
    ob_flush();
    flush();
    return $J;
...
function
apply_sql_function($Fc, $e) {
    return ($Fc ? ($Fc == "unixepoch" ? "DATETIME($e, '$Fc')" : ($Fc == "count distinct" ? "COUNT(DISTINCT " : strtoupper("$Fc("))."$e)") : $e);
}
...
get_temp_dir() {
    ...
    if (!$J) {
        if (function_exists('sys_get_temp_dir')) $J = sys_get_temp_dir(); else {
            $r = @tempnam("", "");
            if (!$r) return
                false;
            $J = dirname($r);
...
function
file_write_unlock($Dc, $xb) {
    rewind($Dc);
    fwrite($Dc, $xb);
    ftruncate($Dc, strlen($xb));
    flock($Dc, LOCK_UN);
    fclose($Dc);
}
...
function
password_file($sb) {
    $r = get_temp_dir()."/adminer.key";
    $J = @file_get_contents($r);
    if ($J || !$sb) return $J;
    $Dc = @fopen($r, "w");
    if ($Dc) {
        chmod($r, 0660);
...
function
select_value($X, $_, $p, $ig) {
    global $b;
    if (is_array($X)) {
        $J = "";
        foreach ($X
                 as $sd => $W) $J .= "<tr>".($X != array_values($X) ? "<th>".h($sd) : "")."<td>".select_value($W, $_, $p, $ig);
        return "<table cellspacing='0'>$J</table>";
    }
    if (!$_) $_ = $b->selectLink($X, $p);
    if ($_ === null) {
    ...
    $J = $b->editVal($X, $p);
    if ($J !== null) {
        if (!is_utf8($J)) $J = "\0"; elseif ($ig != "" && is_shortable($p)) $J = shorten_utf8($J, max(0, +$ig));
        else$J = h($J);
    }
    return $b->selectVal($J, $_, $p, $X);
}
...
is_mail($Rb) {
    ...
    $He = "$Aa+(\\.$Aa+)*@($Ib?\\.)+$Ib";
    return
        is_string($Rb) && preg_match("(^$He(,\\s*$He)*\$)i", $Rb);
}
...
function
count_rows($R, $Z, $pd, $Hc) {
    global $x;
    $G = " FROM ".table($R).($Z ? " WHERE ".implode(" AND ", $Z) : "");
    return ($pd && ($x == "sql" || count($Hc) == 1) ? "SELECT COUNT(DISTINCT ".implode(", ", $Hc).")$G" : "SELECT COUNT(*)".($pd ? " FROM (SELECT 1$G GROUP BY ".implode(", ", $Hc).") x" : $G));
}
...
slow_query($G) {
    ...
    $lg = $b->queryTimeout();
    $Hf = $n->slowQuery($G, $lg);
    if (!$Hf && support("kill") && is_object($i = connect()) && ($m == "" || $i->select_db($m))) {
        $xd = $i->result(connection_id());
        echo '<script', nonce(), '>
...
';
    ...
    ob_flush();
    flush();
    $J = @get_key_vals(($Hf ? $Hf : $G), $i, false);
    if ($i) {
        echo
...
get_token() {
    $Ye = rand(1, 1e6);
    return ($Ye ^ $_SESSION["token"]).":$Ye";
}

function
verify_token() {
    [$tg, $Ye] = explode(":", $_POST["token"]);
    return ($Ye ^ $_SESSION["token"]) == $tg;
}
...
on_help($ib, $Ff = 0) {
    return
        script("mixin(qsl('select, input'), {onmouseover: function (event) { helpMouseover.call(this, event, $ib, $Ff) }, onmouseout: helpMouseout});", "");
}

function
edit_form($a, $q, $K, $Mg) {
    global $b, $x, $tg, $o;
    $bg = $b->tableName(table_status1($a, true));
    page_header(($Mg ? lang(10) : lang(11)), $o, ["select" => [$a, $bg]], $bg);
    if ($K === false) echo "<p class='error'>".lang(12)."\n";
...
';
    if (!$q) echo "<p class='error'>".lang(13)."\n"; else {
        ...
        foreach ($q
            ...
            $Ab = $_GET["set"][bracket_escape($B)];
            if ($Ab === null) {
                $Ab = $p["default"];
                if ($p["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $Ab, $df)) $Ab = $df[1];
            }
            $Y = ($K !== null ? ($K[$B] != "" && $x == "sql" && preg_match("~enum|set~", $p["type"]) ? (is_array($K[$B]) ? array_sum($K[$B]) : +$K[$B]) : $K[$B]) : (!$Mg && $p["auto_increment"] ? "" : (isset($_GET["select"]) ? false : $Ab)));
            if (!$_POST["save"] && is_string($Y)) $Y = $b->editVal($Y, $p);
            $Fc = ($_POST["save"] ? (string)$_POST["function"][$B] : ($Mg && preg_match('~^CURRENT_TIMESTAMP~i', $p["on_update"]) ? "now" : ($Y === false ? null : ($Y !== null ? '' : 'NULL'))));
        ...
        if (!support("table")) echo "<tr>"."<th><input name='field_keys[]'>".script("qsl('input').oninput = fieldChange;")."<td class='function'>".html_select("field_funs[]", $b->editFunctions(["null" => isset($_GET["select"])]))."<td><input name='field_vals[]'>"."\n";
        echo "</table>\n";
    }
    echo "<p>\n";
    if ($q) {
...
if ($_GET["script"] == "version") {
    $Dc = file_open_lock(get_temp_dir()."/adminer.version");
    if ($Dc) file_write_unlock($Dc, serialize(["signature" => $_POST["signature"], "version" => $_POST["version"]]));
    exit;
}
global $b, $h, $n, $Jb, $Ob, $Wb, $o, $Gc, $Kc, $aa, $jd, $x, $ba, $Ad, $ne, $Ie, $Uf, $Oc, $tg, $xg, $Eg, $Lg, $ca;
...
if (!defined("SID")) {
    ...
    $E = [0, preg_replace('~\?.*~', '', $_SERVER["REQUEST_URI"]), "", $aa];
    if (version_compare(PHP_VERSION, '5.2.0') >= 0) $E[] = true;
    call_user_func_array('session_set_cookie_params', $E);
    session_start();
}
...
lang($u, $je = null) {
    if (is_string($u)) {
        $Le = array_search($u, get_translations("en"));
        if ($Le !== false) $u = $Le;
    }
    global $ba, $xg;
    $wg = ($xg[$u] ? $xg[$u] : $u);
    if (is_array($wg)) {
        $Le = ($je == 1 ? 0 : ($ba == 'cs' || $ba == 'sk' ? ($je && $je < 5 ? 1 : 2) : ($ba == 'fr' ? (!$je ? 0 : 1) : ($ba == 'pl' ? ($je % 10 > 1 && $je % 10 < 5 && $je / 10 % 10 != 1 ? 1 : 2) : ($ba == 'sl' ? ($je % 100 == 1 ? 0 : ($je % 100 == 2 ? 1 : ($je % 100 == 3 || $je % 100 == 4 ? 2 : 3))) : ($ba == 'lt' ? ($je % 10 == 1 && $je % 100 != 11 ? 0 : ($je % 10 > 1 && $je / 10 % 10 != 1 ? 1 : 2)) : ($ba == 'bs' || $ba == 'ru' || $ba == 'sr' || $ba == 'uk' ? ($je % 10 == 1 && $je % 100 != 11 ? 0 : ($je % 10 > 1 && $je % 10 < 5 && $je / 10 % 10 != 1 ? 1 : 2)) : 1)))))));
        $wg = $wg[$Le];
    }
    $ya = func_get_args();
    array_shift($ya);
    $Bc = str_replace("%d", "%s", $wg);
    if ($Bc != $wg) $ya[0] = format_number($je);
    return
        vsprintf($Bc, $ya);
}
...
$ba = "en";
if (isset($Ad[$_COOKIE["adminer_lang"]])) {
    cookie("adminer_lang", $_COOKIE["adminer_lang"]);
    $ba = $_COOKIE["adminer_lang"];
} elseif (isset($Ad[$_SESSION["lang"]])) $ba = $_SESSION["lang"];
else {
    $qa = [];
    preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~', str_replace("_", "-", strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])), $Pd, PREG_SET_ORDER);
    foreach ($Pd
             as $A) $qa[$A[1]] = (isset($A[3]) ? $A[3] : 1);
    arsort($qa);
    foreach ($qa
             as $y => $Ve) {
        if (isset($Ad[$y])) {
...
if ($_SESSION["translations_version"] != 1443184115) {
    $xg = [];
    $_SESSION["translations_version"] = 1443184115;
}
function
get_translations($_d) {
    switch ($_d) {
...
if (extension_loaded('pdo')) {
    ...
    Min_PDO
        ...
        __construct() {
            global $b;
            $Le = array_search("SQL", $b->operators);
            if ($Le !== false) unset($b->operators[$Le]);
        }

        function
        dsn($Mb, $V, $F, $C = []) {
            try {
                parent::__construct($Mb, $V, $F, $C);
            } catch (Exception$ac) {
...
Min_SQL {
    ...
    function
    select($R, $M, $Z, $Hc, $ve = [], $z = 1, $D = 0, $Qe = false) {
        global $b, $x;
        $pd = (count($Hc) < count($M));
        $G = $b->selectQueryBuild($M, $Z, $Hc, $ve, $z, $D);
        if (!$G) $G = "SELECT".limit(($_GET["page"] != "last" && $z != "" && $Hc && $pd && $x == "sql" ? "SQL_CALC_FOUND_ROWS " : "").implode(", ", $M)."\nFROM ".table($R), ($Z ? "\nWHERE ".implode(" AND ", $Z) : "").($Hc && $pd ? "\nGROUP BY ".implode(", ", $Hc) : "").($ve ? "\nORDER BY ".implode(", ", $ve) : ""), ($z != "" ? +$z : null), ($D ? $z * $D : 0), "\n");
        $Qf = microtime(true);
        $J = $this->_conn->query($G);
        if ($Qe) echo $b->selectQuery($G, $Qf, !$J);
        return $J;
    }
    ...
    delete($R, $H, $z = 0) {
        $G = "FROM ".table($R);
        return
            queries("DELETE".($z ? limit1($R, $G, $H) : " $G$H"));
    }
    ...
    update($R, $P, $H, $z = 0, $N = "\n") {
        ...
        foreach ($P
                 as $y => $X) $Ug[] = "$y = $X";
        $G = table($R)." SET$N".implode(",$N", $Ug);
        return
            queries("UPDATE".($z ? limit1($R, $G, $H, $N) : " $G$H"));
    }
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"])) {
    $Me = [(isset($_GET["sqlite"]) ? "SQLite3" : "SQLite"), "PDO_SQLite"];
        ...
        if (isset($_GET["sqlite"])) {
            ...
            Min_SQLite {
                ...
                __construct($r) {
                    $this->_link = new
                    SQLite3($r);
                    $Wg = $this->_link->version();
                    $this->server_info = $Wg["versionString"];
                }

                function
                query($G) {
                    $I = @$this->_link->query($G);
                    $this->error = "";
                    if (!$I) {
            ...
            Min_SQLite {
                ...
                function
                query($G, $Fg = false) {
                    $Yd = ($Fg ? "unbufferedQuery" : "query");
                    $I = @$this->_link->$Yd($G, SQLITE_BOTH, $o);
                    $this->error = "";
                    if (!$I) {
                        $this->error = $o;
                        return
            ...
            Min_Result {
                ...
                __construct($I) {
                    $this->_result = $I;
                    if (method_exists($I, 'numRows')) $this->num_rows = $I->numRows();
                }
                ...
                fetch_assoc() {
                    ...
                    $J = [];
                    foreach ($K
                             as $y => $X) $J[($y[0] == '"' ? idf_unescape($y) : $y)] = $X;
                    return $J;
                }
                ...
                fetch_field() {
                    ...
                    if (preg_match("~^($He\\.)?$He\$~", $B, $A)) {
                        $R = ($A[3] != "" ? $A[3] : idf_unescape($A[2]));
                        $B = ($A[5] != "" ? $A[5] : idf_unescape($A[4]));
                    }
                    return (object)["name" => $B, "orgname" => $B, "orgtable" => $R,];
                }
            }
        }
    ...
    if (class_exists("Min_SQLite")) {
        ...
        Min_DB
            ...
            function
            select_db($r) {
                if (is_readable($r) && $this->query("ATTACH ".$this->quote(preg_match("~(^[/\\\\]|:)~", $r) ? $r : dirname($_SERVER["SCRIPT_FILENAME"])."/$r")." AS a")) {
    ...
    function
    limit($G, $Z, $z, $le = 0, $N = " ") {
        return " $G$Z".($z !== null ? $N."LIMIT $z".($le ? " OFFSET $le" : "") : "");
    }

    function
    limit1($R, $G, $Z, $N = "\n") {
        global $h;
        return (preg_match('~^INTO~', $G) || $h->result("SELECT sqlite_compileoption_used('ENABLE_UPDATE_DELETE_LIMIT')") ? limit($G, $Z, 1, 0, $N) : " $G WHERE rowid = (SELECT rowid FROM ".table($R).$Z.$N."LIMIT 1)");
    }
    ...
    table_status($B = "") {
        global $h;
        $J = [];
        foreach (get_rows("SELECT name AS Name, type AS Engine, 'rowid' AS Oid, '' AS Auto_increment FROM sqlite_master WHERE type IN ('table', 'view') ".($B != "" ? "AND name = ".q($B) : "ORDER BY name")) as $K) {
            $K["Rows"] = $h->result("SELECT COUNT(*) FROM ".idf_escape($K["Name"]));
            $J[$K["Name"]] = $K;
        }
        foreach (get_rows("SELECT * FROM sqlite_sequence", null, "") as $K) $J[$K["name"]]["Auto_increment"] = $K["seq"];
        return ($B != "" ? $J[$B] : $J);
    }
    ...
    fields($R) {
        ...
        foreach (get_rows("PRAGMA table_info(".table($R).")") as $K) {
            ...
            $U = strtolower($K["type"]);
            $Ab = $K["dflt_value"];
            $J[$B] = ["field" => $B, "type" => (preg_match('~int~i', $U) ? "integer" : (preg_match('~char|clob|text~i', $U) ? "text" : (preg_match('~blob~i', $U) ? "blob" : (preg_match('~real|floa|doub~i', $U) ? "real" : "numeric")))), "full_type" => $U, "default" => (preg_match("~'(.*)'~", $Ab, $A) ? str_replace("''", "'", $A[1]) : ($Ab == "NULL" ? null : $Ab)), "null" => !$K["notnull"], "privileges" => ["select" => 1, "insert" => 1, "update" => 1], "primary" => $K["pk"],];
            if ($K["pk"]) {
                if ($Oe != "") $J[$Oe]["auto_increment"] = false; elseif (preg_match('~^integer$~i', $U)) $J[$B]["auto_increment"] = true;
                $Oe = $B;
            }
        }
        $Nf = $h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));
        preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i', $Nf, $Pd, PREG_SET_ORDER);
        foreach ($Pd
                 as $A) {
            $B = str_replace('""', '"', preg_replace('~^"|"$~', '', $A[1]));
            if ($J[$B]) $J[$B]["collation"] = trim($A[3], "'");
        }
        return $J;
    }
    ...
    indexes($R, $i = null) {
        ...
        if (!is_object($i)) $i = $h;
        $J = [];
        $Nf = $i->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));
        if (preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*"|`[^`]*`)++)~i', $Nf, $A)) {
            $J[""] = ["type" => "PRIMARY", "columns" => [], "lengths" => [], "descs" => []];
            preg_match_all('~((("[^"]*+")+|(?:`[^`]*+`)+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i', $A[1], $Pd, PREG_SET_ORDER);
            foreach ($Pd
                     as $A) {
                $J[""]["columns"][] = idf_unescape($A[2]).$A[4];
                $J[""]["descs"][] = (preg_match('~DESC~i', $A[5]) ? '1' : null);
            }
        }
        if (!$J) {
            foreach (fields($R) as $B => $p) {
                if ($p["primary"]) $J[""] = ["type" => "PRIMARY", "columns" => [$B], "lengths" => [], "descs" => [null]];
            }
        }
        $Of = get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R), $i);
        foreach (get_rows("PRAGMA index_list(".table($R).")", $i) as $K) {
            $B = $K["name"];
            $v = ["type" => ($K["unique"] ? "UNIQUE" : "INDEX")];
            $v["lengths"] = [];
            $v["descs"] = [];
            foreach (get_rows("PRAGMA index_info(".idf_escape($B).")", $i) as $pf) {
                $v["columns"][] = $pf["name"];
                $v["descs"][] = null;
            }
            if (preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($B).' ON '.idf_escape($R), '~').' \((.*)\)$~i', $Of[$B], $df)) {
                preg_match_all('/("[^"]*+")+( DESC)?/', $df[2], $Pd);
                foreach ($Pd[2] as $y => $X) {
                    if ($X) $v["descs"][$y] = '1';
                }
            }
            if (!$J[""] || $v["type"] != "UNIQUE" || $v["columns"] != $J[""]["columns"] || $v["descs"] != $J[""]["descs"] || !preg_match("~^sqlite_~", $B)) $J[$B] = $v;
        }
        return $J;
    }
    ...
    view($B) {
        global $h;
        return
            ["select" => preg_replace('~^(?:[^`"[]+|`[^`]*`|"[^"]*")* AS\s+~iU', '', $h->result("SELECT sql FROM sqlite_master WHERE name = ".q($B)))];
    }
    ...
    check_sqlite_name($B) {
        global $h;
        $hc = "db|sdb|sqlite";
        if (!preg_match("~^[^\\0]*\\.($hc)\$~", $B)) {
            $h->error = lang(23, str_replace("|", ", ", $hc));
            return
                false;
        }
        return
    ...
    function
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        global $h;
        $Qg = ($R == "" || $xc);
        foreach ($q
                 as $p) {
        ...
        if (!$Qg) {
            ...
            if ($R != $B && !queries("ALTER TABLE ".table($R)." RENAME TO ".table($B))) return
                false;
        } elseif (!recreate_table($R, $B, $c, $ye, $xc, $Ga)) return
            false;
        if ($Ga) {
            queries("BEGIN");
            queries("UPDATE sqlite_sequence SET seq = $Ga WHERE name = ".q($B));
            if (!$h->affected_rows) queries("INSERT INTO sqlite_sequence (name, seq) VALUES (".q($B).", $Ga)");
            queries("COMMIT");
        }
        return
            true;
    ...
    function
    recreate_table($R, $B, $q, $ye, $xc, $Ga, $w = []) {
        global $h;
        if ($R != "") {
            if (!$q) {
                foreach (fields($R) as $y => $p) {
                    if ($w) $p["auto_increment"] = 0;
                    $q[] = process_field($p, $p);
                    $ye[$y] = idf_escape($y);
                }
            }
            ...
            foreach (indexes($R) as $vd => $v) {
                $f = [];
                foreach ($v["columns"] as $y => $e) {
                    if (!$ye[$e]) continue
                    2;
                    $f[] = $ye[$e].($v["descs"][$y] ? " DESC" : "");
                }
                if (!$Lb[$vd]) {
                    if ($v["type"] != "PRIMARY" || !$Pe) $w[] = [$v["type"], $vd, $f];
                }
            }
            foreach ($w
                     as $y => $X) {
                if ($X[0] == "PRIMARY") {
                    unset($w[$y]);
                    $xc[] = "  PRIMARY KEY (".implode(", ", $X[2]).")";
                }
            }
            foreach (foreign_keys($R) as $vd => $_c) {
                foreach ($_c["source"] as $y => $e) {
                    if (!$ye[$e]) continue
                    2;
                    $_c["source"][$y] = idf_unescape($ye[$e]);
                }
                if (!isset($xc[" $vd"])) $xc[] = " ".format_foreign_key($_c);
            }
            queries("BEGIN");
        }
        foreach ($q
                 as $y => $p) $q[$y] = "  ".implode($p);
        $q = array_merge($q, array_filter($xc));
        $fg = ($R == $B ? "adminer_$B" : $B);
        if (!queries("CREATE TABLE ".table($fg)." (\n".implode(",\n", $q)."\n)")) return
            false;
        if ($R != "") {
            if ($ye && !queries("INSERT INTO ".table($fg)." (".implode(", ", $ye).") SELECT ".implode(", ", array_map('idf_escape', array_keys($ye)))." FROM ".table($R))) return
                false;
            $Cg = [];
            foreach (triggers($R) as $Ag => $mg) {
                $_g = trigger($Ag);
                $Cg[] = "CREATE TRIGGER ".idf_escape($Ag)." ".implode(" ", $mg)." ON ".table($B)."\n$_g[Statement]";
            }
            $Ga = $Ga ? 0 : $h->result("SELECT seq FROM sqlite_sequence WHERE name = ".q($R));
            if (!queries("DROP TABLE ".table($R)) || ($R == $B && !queries("ALTER TABLE ".table($fg)." RENAME TO ".table($B))) || !alter_indexes($B, $w)) return
                false;
            if ($Ga) queries("UPDATE sqlite_sequence SET seq = $Ga WHERE name = ".q($B));
            foreach ($Cg
                     as $_g) {
                if (!queries($_g)) return
                    false;
            }
            queries("COMMIT");
        }
    ...
    function
    index_sql($R, $U, $B, $f) {
        return "CREATE $U ".($U != "INDEX" ? "INDEX " : "").idf_escape($B != "" ? $B : uniqid($R."_"))." ON ".table($R)." $f";
    }
    ...
    alter_indexes($R, $c) {
        foreach ($c
                 as $Oe) {
            if ($Oe[0] == "PRIMARY") return
                recreate_table($R, $R, [], [], [], 0, $c);
        }
    ...
    trigger($B) {
        ...
        $u = '(?:[^`"\s]+|`[^`]*`|"[^"]*")+';
        $Bg = trigger_options();
        preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|", $Bg["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is", $h->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($B)), $A);
        $ke = $A[3];
        return
            ["Timing" => strtoupper($A[1]), "Event" => strtoupper($A[2]).($ke ? " OF" : ""), "Of" => ($ke[0] == '`' || $ke[0] == '"' ? idf_unescape($ke) : $ke), "Trigger" => $B, "Statement" => $A[4],];
    }
    ...
    triggers($R) {
        ...
        foreach (get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($R)) as $K) {
            preg_match('~^CREATE\s+TRIGGER\s*(?:[^`"\s]+|`[^`]*`|"[^"]*")+\s*('.implode("|", $Bg["Timing"]).')\s*(.*?)\s+ON\b~i', $K["sql"], $A);
            $J[$K["name"]] = [$A[1], $A[2]];
        }
        return $J;
    }
    ...
    create_sql($R, $Ga, $Vf) {
        global $h;
        $J = $h->result("SELECT sql FROM sqlite_master WHERE type IN ('table', 'view') AND name = ".q($R));
        foreach (indexes($R) as $B => $v) {
            if ($B == '') continue;
            $J .= ";\n\n".index_sql($R, $v['type'], $B, "(".implode(", ", array_map('idf_escape', $v['columns'])).")");
        }
        return $J;
    }
    ...
    show_status() {
        $J = [];
        foreach (get_vals("PRAGMA compile_options") as $te) {
            [$y, $X] = explode("=", $te, 2);
            $J[$y] = $X;
        }
        return $J;
    ...
    support($lc) {
        return
            preg_match('~^(columns|database|drop_col|dump|indexes|descidx|move_col|sql|status|table|trigger|variables|view|view_trigger)$~', $lc);
    }

    $x = "sqlite";
    $Eg = ["integer" => 0, "real" => 0, "numeric" => 0, "text" => 0, "blob" => 0];
    $Uf = array_keys($Eg);
    $Lg = [];
    $se = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL", "SQL"];
    $Gc = ["hex", "length", "lower", "round", "unixepoch", "upper"];
    $Kc = ["avg", "count", "count distinct", "group_concat", "max", "min", "sum"];
    $Ob = [[], ["integer|real|numeric" => "+/-", "text" => "||",]];
}
$Jb["pgsql"] = "PostgreSQL";
if (isset($_GET["pgsql"])) {
    $Me = ["PgSQL", "PDO_PgSQL"];
    ...
    if (extension_loaded("pgsql")) {
        ...
        Min_DB {
            ...
            connect($O, $V, $F) {
                ...
                $this->_string = "host='".str_replace(":", "' port='", addcslashes($O, "'\\"))."' user='".addcslashes($V, "'\\")."' password='".addcslashes($F, "'\\")."'";
                ...
                if ($this->_link) {
                    $Wg = pg_version($this->_link);
                    $this->server_info = $Wg["server"];
            ...
            result($G, $p = 0) {
                $I = $this->query($G);
                if (!$I || !$I->num_rows) return
                    false;
                return
                    pg_fetch_result($I->_result, 0, $p);
            }
        ...
        Min_Result {
            ...
            fetch_field() {
                ...
                $J = new
                stdClass;
                if (function_exists('pg_field_table')) $J->orgtable = pg_field_table($this->_result, $e);
        ...
        Min_DB
            ...
            connect($O, $V, $F) {
                global $b;
                $m = $b->database();
                $Q = "pgsql:host='".str_replace(":", "' port='", addcslashes($O, "'\\"))."' options='-c client_encoding=utf8'";
                $this->dsn("$Q dbname='".($m != "" ? addcslashes($m, "'\\") : "postgres")."'", $V, $F);
                return
                    true;
            }
    ...
    Min_Driver
        ...
        insertUpdate($R, $L, $Oe) {
            ...
            foreach ($L
                ...
                foreach ($P
                         as $y => $X) {
                    $Mg[] = "$y = $X";
                    if (isset($Oe[idf_unescape($y)])) $Z[] = "$y = $X";
                }
                if (!(($Z && queries("UPDATE ".table($R)." SET ".implode(", ", $Mg)." WHERE ".implode(" AND ", $Z)) && $h->affected_rows) || queries("INSERT INTO ".table($R)." (".implode(", ", array_keys($P)).") VALUES (".implode(", ", $P).")"))) return
                    false;
            }
            return
                true;
        ...
        function
        slowQuery($G, $lg) {
            $this->_conn->query("SET statement_timeout = ".(1000 * $lg));
            $this->_conn->timeout = 1000 * $lg;
            return $G;
        }

        function
        convertSearch($u, $X, $p) {
            return (preg_match('~char|text'.(!preg_match('~LIKE~', $X["op"]) ? '|date|time(stamp)?|boolean|uuid|'.number_type() : '').'~', $p["type"]) ? $u : "CAST($u AS text)");
        }

        function
        quoteBinary($qf) {
            return $this->_conn->quoteBinary($qf);
        }
        ...
        function
        tableHelp($B) {
            $Hd = ["information_schema" => "infoschema", "pg_catalog" => "catalog",];
            $_ = $Hd[$_GET["ns"]];
    ...
    function
    limit1($R, $G, $Z, $N = "\n") {
        return (preg_match('~^INTO~', $G) ? limit($G, $Z, 1, 0, $N) : " $G".(is_view(table_status1($R)) ? $Z : " WHERE ctid = (SELECT ctid FROM ".table($R).$Z.$N."LIMIT 1)"));
    }
...
".($B != "" ? "AND relname = ".q($B) : "ORDER BY relname")) as $K) $J[$K["Name"]] = $K;
        return ($B != "" ? $J[$B] : $J);
    }
    ...
    fields($R) {
        $J = [];
        $xa = ['timestamp without time zone' => 'timestamp', 'timestamp with time zone' => 'timestamptz',];
        $Xc = min_version(10) ? "(a.attidentity = 'd')::int" : '0';
        foreach (get_rows("SELECT a.attname AS field, format_type(a.atttypid, a.atttypmod) AS full_type, pg_get_expr(d.adbin, d.adrelid) AS default, a.attnotnull::int, col_description(c.oid, a.attnum) AS comment, $Xc AS identity
FROM pg_class c
...
ORDER BY a.attnum") as $K) {
            preg_match('~([^([]+)(\((.*)\))?([a-z ]+)?((\[[0-9]*])*)$~', $K["full_type"], $A);
            [, $U, $Ed, $K["length"], $sa, $za] = $A;
            $K["length"] .= $za;
            $Wa = $U.$sa;
            if (isset($xa[$Wa])) {
                $K["type"] = $xa[$Wa];
                $K["full_type"] = $K["type"].$Ed.$za;
            } else {
                $K["type"] = $U;
                $K["full_type"] = $K["type"].$Ed.$sa.$za;
            }
            if ($K['identity']) $K['default'] = 'GENERATED BY DEFAULT AS IDENTITY';
            $K["null"] = !$K["attnotnull"];
            $K["auto_increment"] = $K['identity'] || preg_match('~^nextval\(~i', $K["default"]);
            $K["privileges"] = ["insert" => 1, "select" => 1, "update" => 1];
            if (preg_match('~(.+)::[^)]+(.*)~', $K["default"], $A)) $K["default"] = ($A[1] == "NULL" ? null : (($A[1][0] == "'" ? idf_unescape($A[1]) : $A[1]).$A[2]));
            $J[$K["field"]] = $K;
        }
        return $J;
    }
    ...
    indexes($R, $i = null) {
        ...
        if (!is_object($i)) $i = $h;
        $J = [];
        $cg = $i->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($R));
        $f = get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $cg AND attnum > 0", $i);
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $cg AND ci.oid = i.indexrelid", $i) as $K) {
            $ef = $K["relname"];
            $J[$ef]["type"] = ($K["indispartial"] ? "INDEX" : ($K["indisprimary"] ? "PRIMARY" : ($K["indisunique"] ? "UNIQUE" : "INDEX")));
            $J[$ef]["columns"] = [];
            foreach (explode(" ", $K["indkey"]) as $fd) $J[$ef]["columns"][] = $f[$fd];
            $J[$ef]["descs"] = [];
            foreach (explode(" ", $K["indoption"]) as $gd) $J[$ef]["descs"][] = ($gd & 1 ? '1' : null);
            $J[$ef]["lengths"] = [];
        }
        return $J;
    }
...
ORDER BY conkey, conname") as $K) {
            if (preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA', $K['definition'], $A)) {
                $K['source'] = array_map('trim', explode(',', $A[1]));
                if (preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~', $A[2], $Od)) {
                    $K['ns'] = str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $Od[2]));
                    $K['table'] = str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $Od[4]));
                }
                $K['target'] = array_map('trim', explode(',', $A[3]));
                $K['on_delete'] = (preg_match("~ON DELETE ($ne)~", $A[4], $Od) ? $Od[1] : 'NO ACTION');
                $K['on_update'] = (preg_match("~ON UPDATE ($ne)~", $A[4], $Od) ? $Od[1] : 'NO ACTION');
                $J[$K['conname']] = $K;
            }
        }
        return $J;
    ...
    view($B) {
        global $h;
        return
            ["select" => trim($h->result("SELECT pg_get_viewdef(".$h->result("SELECT oid FROM pg_class WHERE relname = ".q($B)).")"))];
    }
    ...
    error() {
        global $h;
        $J = h($h->error);
        if (preg_match('~^(.*\n)?([^\n]*)\n( *)\^(\n.*)?$~s', $J, $A)) $J = $A[1].preg_replace('~((?:[^&]|&[^;]*;){'.strlen($A[3]).'})(.*)~', '\1<b>\2</b>', $A[2]).$A[4];
        return
            nl_br($J);
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        $c = [];
        $We = [];
        if ($R != "" && $R != $B) $We[] = "ALTER TABLE ".table($R)." RENAME TO ".table($B);
        foreach ($q
                 as $p) {
            $e = idf_escape($p[0]);
            $X = $p[1];
            if (!$X) $c[] = "DROP $e"; else {
                ...
                if ($p[0] == "") $c[] = ($R != "" ? "ADD " : "  ").implode($X); else {
                    if ($e != $X[0]) $We[] = "ALTER TABLE ".table($B)." RENAME $e TO $X[0]";
                    $c[] = "ALTER $e TYPE$X[1]";
                    if (!$X[6]) {
                ...
                if ($p[0] != "" || $Tg != "") $We[] = "COMMENT ON COLUMN ".table($B).".$X[0] IS ".($Tg != "" ? substr($Tg, 9) : "''");
            }
        }
        $c = array_merge($c, $xc);
        if ($R == "") array_unshift($We, "CREATE TABLE ".table($B)." (\n".implode(",\n", $c)."\n)"); elseif ($c) array_unshift($We, "ALTER TABLE ".table($R)."\n".implode(",\n", $c));
        if ($R != "" || $jb != "") $We[] = "COMMENT ON TABLE ".table($B)." IS ".q($jb);
        if ($Ga != "") {
        }
        foreach ($We
                 as $G) {
    ...
    alter_indexes($R, $c) {
        ...
        foreach ($c
                 as $X) {
            if ($X[0] != "INDEX") $sb[] = ($X[2] == "DROP" ? "\nDROP CONSTRAINT ".idf_escape($X[1]) : "\nADD".($X[1] != "" ? " CONSTRAINT ".idf_escape($X[1]) : "")." $X[0] ".($X[0] == "PRIMARY" ? "KEY " : "")."(".implode(", ", $X[2]).")"); elseif ($X[2] == "DROP") $Kb[] = idf_escape($X[1]);
            else$We[] = "CREATE INDEX ".idf_escape($X[1] != "" ? $X[1] : uniqid($R."_"))." ON ".table($R)." (".implode(", ", $X[2]).")";
        }
        if ($sb) array_unshift($We, "ALTER TABLE ".table($R).implode(",", $sb));
        if ($Kb) array_unshift($We, "DROP INDEX ".implode(", ", $Kb));
        foreach ($We
                 as $G) {
            if (!queries($G)) return
                false;
    ...
    drop_tables($T) {
        foreach ($T
                 as $R) {
            $Sf = table_status($R);
            if (!queries("DROP ".strtoupper($Sf["Engine"])." ".table($R))) return
                false;
        }
        return
            true;
    ...
    function
    move_tables($T, $Yg, $eg) {
        foreach (array_merge($T, $Yg) as $R) {
            $Sf = table_status($R);
            if (!queries("ALTER ".strtoupper($Sf["Engine"])." ".table($R)." SET SCHEMA ".idf_escape($eg))) return
                false;
        }
        return
            true;
    ...
    triggers($R) {
        $J = [];
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = ".q($R)) as $K) $J[$K["trigger_name"]] = [$K["action_timing"], $K["event_manipulation"]];
        return $J;
    }
...
ORDER BY SPECIFIC_NAME');
    ...
    routine_id($B, $K) {
        $J = [];
        foreach ($K["fields"] as $p) $J[] = $p["type"];
        return
            idf_escape($B)."(".implode(", ", $J).")";
    }
    ...
    found_rows($S, $Z) {
        global $h;
        if (preg_match("~ rows=([0-9]+)~", $h->result("EXPLAIN SELECT * FROM ".idf_escape($S["Name"]).($Z ? " WHERE ".implode(" AND ", $Z) : "")), $df)) return $df[1];
        return
            false;
    }
...
AND typelem = 0");
    ...
    function
    set_schema($rf, $i = null) {
        global $h, $Eg, $Uf;
        if (!$i) $i = $h;
        $J = $i->query("SET search_path TO ".idf_escape($rf));
        foreach (types() as $U) {
            if (!isset($Eg[$U])) {
                $Eg[$U] = 0;
                $Uf[lang(26)][] = $U;
    ...
    create_sql($R, $Ga, $Vf) {
        ...
        if (is_view($Sf)) {
            $Xg = view($R);
            return
                rtrim("CREATE VIEW ".idf_escape($R)." AS $Xg[select]", ";");
        }
        $q = fields($R);
        $w = indexes($R);
        ksort($w);
        $uc = foreign_keys($R);
        ksort($uc);
        if (!$Sf || empty($q)) return
            false;
        $J = "CREATE TABLE ".idf_escape($Sf['nspname']).".".idf_escape($Sf['Name'])." (\n    ";
        foreach ($q
                 as $mc => $p) {
            $De = idf_escape($p['field']).' '.$p['full_type'].default_value($p).($p['attnotnull'] ? " NOT NULL" : "");
            $nf[] = $De;
            if (preg_match('~nextval\(\'([^\']+)\'\)~', $p['default'], $Pd)) {
                $_f = $Pd[1];
                $Mf = reset(get_rows(min_version(10) ? "SELECT *, cache_size AS cache_value FROM pg_sequences WHERE schemaname = current_schema() AND sequencename = ".q($_f) : "SELECT * FROM $_f"));
                $Af[] = ($Vf == "DROP+CREATE" ? "DROP SEQUENCE IF EXISTS $_f;\n" : "")."CREATE SEQUENCE $_f INCREMENT $Mf[increment_by] MINVALUE $Mf[min_value] MAXVALUE $Mf[max_value] START ".($Ga ? $Mf['last_value'] : 1)." CACHE $Mf[cache_value];";
            }
        }
        if (!empty($Af)) $J = implode("\n\n", $Af)."\n\n$J";
        foreach ($w
                 as $ad => $v) {
            switch ($v['type']) {
                case'UNIQUE':
                    $nf[] = "CONSTRAINT ".idf_escape($ad)." UNIQUE (".implode(', ', array_map('idf_escape', $v['columns'])).")";
                    break;
                case'PRIMARY':
                    $nf[] = "CONSTRAINT ".idf_escape($ad)." PRIMARY KEY (".implode(', ', array_map('idf_escape', $v['columns'])).")";
                    break;
            }
        }
        foreach ($uc
                 as $tc => $sc) $nf[] = "CONSTRAINT ".idf_escape($tc)." $sc[definition] ".($sc['deferrable'] ? 'DEFERRABLE' : 'NOT DEFERRABLE');
        $J .= implode(",\n    ", $nf)."\n) WITH (oids = ".($Sf['Oid'] ? 'true' : 'false').");";
        foreach ($w
                 as $ad => $v) {
            if ($v['type'] == 'INDEX') {
                $f = [];
                foreach ($v['columns'] as $y => $X) $f[] = idf_escape($X).($v['descs'][$y] ? " DESC" : "");
                $J .= "\n\nCREATE INDEX ".idf_escape($ad)." ON ".idf_escape($Sf['nspname']).".".idf_escape($Sf['Name'])." USING btree (".implode(', ', $f).");";
            }
        }
        if ($Sf['Comment']) $J .= "\n\nCOMMENT ON TABLE ".idf_escape($Sf['nspname']).".".idf_escape($Sf['Name'])." IS ".q($Sf['Comment']).";";
        foreach ($q
                 as $mc => $p) {
            if ($p['comment']) $J .= "\n\nCOMMENT ON COLUMN ".idf_escape($Sf['nspname']).".".idf_escape($Sf['Name']).".".idf_escape($mc)." IS ".q($p['comment']).";";
        }
        return
            rtrim($J, ';');
    }
    ...
    trigger_sql($R) {
        $Sf = table_status($R);
        $J = "";
        foreach (triggers($R) as $zg => $yg) {
            $_g = trigger($zg, $Sf['Name']);
    ...
    support($lc) {
        return
            preg_match('~^(database|table|columns|sql|indexes|descidx|comment|view|'.(min_version(9.3) ? 'materializedview|' : '').'scheme|routine|processlist|sequence|trigger|type|variables|drop_col|kill|dump)$~', $lc);
    }
    ...
    $Lg = [];
    $se = ["=", "<", ">", "<=", ">=", "!=", "~", "!~", "LIKE", "LIKE %%", "ILIKE", "ILIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL"];
    $Gc = ["char_length", "lower", "round", "to_hex", "to_timestamp", "upper"];
    $Kc = ["avg", "count", "count distinct", "max", "min", "sum"];
    $Ob = [["char" => "md5", "date|time" => "now",], [number_type() => "+/-", "date|time" => "+ interval/- interval", "char|text" => "||",]];
}
$Jb["oracle"] = "Oracle (beta)";
if (isset($_GET["oracle"])) {
    ...
    if (extension_loaded("oci8")) {
        ...
        Min_DB {
            ...
            function
            connect($O, $V, $F) {
                $this->_link = @oci_new_connect($V, $F, $O, "AL32UTF8");
                if ($this->_link) {
            ...
            result($G, $p = 1) {
                $I = $this->query($G);
                if (!is_object($I) || !oci_fetch($I->_result)) return
                    false;
                return
                    oci_result($I->_result, $p);
            }
        }
        ...
        Min_DB
            ...
            function
            connect($O, $V, $F) {
                $this->dsn("oci:dbname=//$O;charset=AL32UTF8", $V, $F);
                return
                    true;
            }
    ...
    connect() {
        ...
        Min_DB;
        $j = $b->credentials();
        if ($h->connect($j[0], $j[1], $j[2])) return $h;
        return $h->error;
    }
    ...
    function
    limit($G, $Z, $z, $le = 0, $N = " ") {
        return ($le ? " * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $G$Z) t WHERE rownum <= ".($z + $le).") WHERE rnum > $le" : ($z !== null ? " * FROM (SELECT $G$Z) WHERE rownum <= ".($z + $le) : " $G$Z"));
    }
...
ORDER BY 1");
    ...
    table_status($B = "") {
        $J = [];
        $tf = q($B);
        foreach (get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($B != "" ? " AND table_name = $tf" : "")."
...
ORDER BY 1") as $K) {
    ...
    fields($R) {
        ...
        foreach (get_rows("SELECT * FROM all_tab_columns WHERE table_name = ".q($R)." ORDER BY column_id") as $K) {
            $U = $K["DATA_TYPE"];
            $Ed = "$K[DATA_PRECISION],$K[DATA_SCALE]";
            if ($Ed == ",") $Ed = $K["DATA_LENGTH"];
            $J[$K["COLUMN_NAME"]] = ["field" => $K["COLUMN_NAME"], "full_type" => $U.($Ed ? "($Ed)" : ""), "type" => strtolower($U), "length" => $Ed, "default" => $K["DATA_DEFAULT"], "null" => ($K["NULLABLE"] == "Y"), "privileges" => ["insert" => 1, "select" => 1, "update" => 1],];
        }
        return $J;
    }
...
ORDER BY uc.constraint_type, uic.column_position", $i) as $K) {
            $ad = $K["INDEX_NAME"];
            $J[$ad]["type"] = ($K["CONSTRAINT_TYPE"] == "P" ? "PRIMARY" : ($K["CONSTRAINT_TYPE"] == "U" ? "UNIQUE" : "INDEX"));
            $J[$ad]["columns"][] = $K["COLUMN_NAME"];
            $J[$ad]["lengths"][] = ($K["CHAR_LENGTH"] && $K["CHAR_LENGTH"] != $K["COLUMN_LENGTH"] ? $K["CHAR_LENGTH"] : null);
            $J[$ad]["descs"][] = ($K["DESCEND"] ? '1' : null);
        }
        return $J;
    }
    ...
    function
    view($B) {
        $L = get_rows('SELECT text "select" FROM user_views WHERE view_name = '.q($B));
        return
            reset($L);
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        ...
        foreach ($q
                 as $p) {
            $X = $p[1];
            if ($X && $p[0] != "" && idf_escape($p[0]) != $X[0]) queries("ALTER TABLE ".table($R)." RENAME COLUMN ".idf_escape($p[0])." TO $X[0]");
            if ($X) $c[] = ($R != "" ? ($p[0] != "" ? "MODIFY (" : "ADD (") : "  ").implode($X).($R != "" ? ")" : ""); else$Kb[] = idf_escape($p[0]);
        }
        if ($R == "") return
            queries("CREATE TABLE ".table($B)." (\n".implode(",\n", $c)."\n)");
        return (!$c || queries("ALTER TABLE ".table($R)."\n".implode("\n", $c))) && (!$Kb || queries("ALTER TABLE ".table($R)." DROP (".implode(", ", $Kb).")")) && ($R == $B || queries("ALTER TABLE ".table($R)." RENAME TO ".table($B)));
    }
...
AND c_src.TABLE_NAME = ".q($R);
        foreach (get_rows($G) as $K) $J[$K['NAME']] = ["db" => $K['DEST_DB'], "table" => $K['DEST_TABLE'], "source" => [$K['SRC_COLUMN']], "target" => [$K['DEST_COLUMN']], "on_delete" => $K['ON_DELETE'], "on_update" => null,];
        return $J;
    }
    ...
    function
    set_schema($sf, $i = null) {
        global $h;
        if (!$i) $i = $h;
        return $i->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($sf));
    }
...
');
    ...
    support($lc) {
        return
            preg_match('~^(columns|database|drop_col|indexes|descidx|processlist|scheme|sql|status|table|variables|view|view_trigger)$~', $lc);
    }
    ...
    $Lg = [];
    $se = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL"];
    $Gc = ["length", "lower", "round", "upper"];
    $Kc = ["avg", "count", "count distinct", "max", "min", "sum"];
    $Ob = [["date" => "current_date", "timestamp" => "current_timestamp",], ["number|float|double" => "+/-", "date|timestamp" => "+ interval/- interval", "char|clob" => "||",]];
}
$Jb["mssql"] = "MS SQL (beta)";
if (isset($_GET["mssql"])) {
    $Me = ["SQLSRV", "MSSQL", "PDO_DBLIB"];
    define("DRIVER", "mssql");
    if (extension_loaded("sqlsrv")) {
        ...
        Min_DB {
            ...
            connect($O, $V, $F) {
                global $b;
                $m = $b->database();
                $nb = ["UID" => $V, "PWD" => $F, "CharacterSet" => "UTF-8"];
                if ($m != "") $nb["Database"] = $m;
                $this->_link = @sqlsrv_connect(preg_replace('~:~', ',', $O), $nb);
                if ($this->_link) {
                    $hd = sqlsrv_server_info($this->_link);
                    $this->server_info = $hd['SQLServerVersion'];
        ...
        Min_Result {
            ...
            fetch_field() {
                ...
                $J->name = $p["Name"];
                $J->orgname = $p["Name"];
                $J->type = ($p["Type"] == 1 ? 254 : 0);
                return $J;
            }

            function
            seek($le) {
                for ($s = 0; $s < $le; $s++) sqlsrv_fetch($this->_result);
            }
        ...
        Min_DB {
            ...
            function
            connect($O, $V, $F) {
                $this->_link = @mssql_connect($O, $V, $F);
                if ($this->_link) {
        ...
        Min_Result {
            ...
            function
            seek($le) {
                mssql_data_seek($this->_result, $le);
            }
        ...
        Min_DB
            ...
            function
            connect($O, $V, $F) {
                $this->dsn("dblib:charset=utf8;host=".str_replace(":", ";unix_socket=", preg_replace('~:(\d)~', ';port=\1', $O)), $V, $F);
                return
                    true;
            }
    ...
    Min_Driver
        ...
        insertUpdate($R, $L, $Oe) {
            foreach ($L
                ...
                if (!queries("MERGE ".table($R)." USING (VALUES(".implode(", ", $P).")) AS source (c".implode(", c", range(1, count($P))).") ON ".implode(" AND ", $Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ", $Mg)." WHEN NOT MATCHED THEN INSERT (".implode(", ", array_keys($P)).") VALUES (".implode(", ", $P).");")) return
                    false;
            }
            return
                true;
    ...
    count_tables($l) {
        ...
        foreach ($l
                 as $m) {
            $h->select_db($m);
            $J[$m] = $h->result("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES");
        }
        return $J;
    }
    ...
    table_status($B = "") {
        $J = [];
        foreach (get_rows("SELECT ao.name AS Name, ao.type_desc AS Engine, (SELECT value FROM fn_listextendedproperty(default, 'SCHEMA', schema_name(schema_id), 'TABLE', ao.name, null, null)) AS Comment FROM sys.all_objects AS ao WHERE schema_id = SCHEMA_ID(".q(get_schema()).") AND type IN ('S', 'U', 'V') ".($B != "" ? "AND name = ".q($B) : "ORDER BY name")) as $K) {
            if ($B != "") return $K;
            $J[$K["Name"]] = $K;
        }
        return $J;
    ...
    function
    fields($R) {
        $kb = get_key_vals("SELECT objname, cast(value as varchar) FROM fn_listextendedproperty('MS_DESCRIPTION', 'schema', ".q(get_schema()).", 'table', ".q($R).", 'column', NULL)");
        $J = [];
...
WHERE o.schema_id = SCHEMA_ID(".q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($R)) as $K) {
            $U = $K["type"];
            $Ed = (preg_match("~char|binary~", $U) ? $K["max_length"] : ($U == "decimal" ? "$K[precision],$K[scale]" : ""));
            $J[$K["name"]] = ["field" => $K["name"], "full_type" => $U.($Ed ? "($Ed)" : ""), "type" => $U, "length" => $Ed, "default" => $K["default"], "null" => $K["is_nullable"], "auto_increment" => $K["is_identity"], "collation" => $K["collation_name"], "privileges" => ["insert" => 1, "select" => 1, "update" => 1], "primary" => $K["is_identity"], "comment" => $kb[$K["name"]],];
        }
        return $J;
    }
...
WHERE OBJECT_NAME(i.object_id) = ".q($R), $i) as $K) {
            $B = $K["name"];
            $J[$B]["type"] = ($K["is_primary_key"] ? "PRIMARY" : ($K["is_unique"] ? "UNIQUE" : "INDEX"));
            $J[$B]["lengths"] = [];
            $J[$B]["columns"][$K["key_ordinal"]] = $K["column_name"];
    ...
    view($B) {
        global $h;
        return
            ["select" => preg_replace('~^(?:[^[]|\[[^]]*])*\s+AS\s+~isU', '', $h->result("SELECT VIEW_DEFINITION FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = SCHEMA_NAME() AND TABLE_NAME = ".q($B)))];
    }

    function
    collations() {
        $J = [];
        foreach (get_vals("SELECT name FROM fn_helpcollations()") as $d) $J[preg_replace('~_.*~', '', $d)][] = $d;
        return $J;
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        ...
        foreach ($q
            ...
            if (!$X) $c["DROP"][] = " COLUMN $e"; else {
                ...
                $kb[$p[0]] = $X[5];
                unset($X[5]);
                if ($p[0] == "") $c["ADD"][] = "\n  ".implode("", $X).($R == "" ? substr($xc[$X[0]], 16 + strlen($X[0])) : ""); else {
                    unset($X[6]);
                    if ($e != $X[0]) queries("EXEC sp_rename ".q(table($R).".$e").", ".q(idf_unescape($X[0])).", 'COLUMN'");
        ...
        if ($R != $B) queries("EXEC sp_rename ".q(table($R)).", ".q($B));
        if ($xc) $c[""] = $xc;
        foreach ($c
                 as $y => $X) {
        ...
        foreach ($kb
            ...
            queries("EXEC sp_addextendedproperty @name = N'MS_Description', @value = ".$jb.", @level0type = N'Schema', @level0name = ".q(get_schema()).", @level1type = N'Table',  @level1name = ".q($B).", @level2type = N'Column', @level2name = ".q($y));
        }
        return
            true;
    }
    ...
    alter_indexes($R, $c) {
        ...
        foreach ($c
                 as $X) {
            if ($X[2] == "DROP") {
                if ($X[0] == "PRIMARY") $Kb[] = idf_escape($X[1]); else$v[] = idf_escape($X[1])." ON ".table($R);
        ...
        return (!$v || queries("DROP INDEX ".implode(", ", $v))) && (!$Kb || queries("ALTER TABLE ".table($R)." DROP ".implode(", ", $Kb)));
    }
    ...
    foreign_keys($R) {
        ...
        foreach (get_rows("EXEC sp_fkeys @fktable_name = ".q($R)) as $K) {
            $_c =& $J[$K["FK_NAME"]];
            $_c["db"] = $K["PKTABLE_QUALIFIER"];
            $_c["table"] = $K["PKTABLE_NAME"];
            $_c["source"][] = $K["FKCOLUMN_NAME"];
            $_c["target"][] = $K["PKCOLUMN_NAME"];
        }
        return $J;
    }
    ...
    move_tables($T, $Yg, $eg) {
        return
            apply_queries("ALTER SCHEMA ".idf_escape($eg)." TRANSFER", array_merge($T, $Yg));
    }
...
WHERE sys1.xtype = 'TR' AND sys2.name = ".q($R)) as $K) $J[$K["name"]] = [$K["Timing"], $K["Event"]];
        return $J;
    }
    ...
    support($lc) {
        return
            preg_match('~^(comment|columns|database|drop_col|indexes|descidx|scheme|sql|table|trigger|view|view_trigger)$~', $lc);
    }
    ...
    $Lg = [];
    $se = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL"];
    $Gc = ["len", "lower", "round", "upper"];
    $Kc = ["avg", "count", "count distinct", "max", "min", "sum"];
    $Ob = [["date|time" => "getdate",], ["int|decimal|real|float|money|datetime" => "+/-", "char|text" => "+",]];
}
$Jb['firebird'] = 'Firebird (alpha)';
if (isset($_GET["firebird"])) {
    ...
    if (extension_loaded("interbase")) {
        ...
        Min_DB {
            ...
            function
            connect($O, $V, $F) {
                $this->_link = ibase_connect($O, $V, $F);
                if ($this->_link) {
                    $Pg = explode(':', $O);
                    $this->service_link = ibase_service_attach($Pg[0], $V, $F);
        ...
        Min_Result {
            ...
            fetch_field() {
                ...
                return (object)['name' => $p['name'], 'orgname' => $p['name'], 'type' => $p['type'], 'charsetnr' => $p['length'],];
            }
    ...
    tables_list() {
        ...
        $I = ibase_query($h->_link, $G);
        $J = [];
        while ($K = ibase_fetch_assoc($I)) $J[$K['RDB$RELATION_NAME']] = 'table';
        ksort($J);
        return $J;
    }
...
ORDER BY r.RDB$FIELD_POSITION';
        $I = ibase_query($h->_link, $G);
        while ($K = ibase_fetch_assoc($I)) $J[trim($K['FIELD_NAME'])] = ["field" => trim($K["FIELD_NAME"]), "full_type" => trim($K["FIELD_TYPE"]), "type" => trim($K["FIELD_SUB_TYPE"]), "default" => trim($K['FIELD_DEFAULT_VALUE']), "null" => (trim($K["FIELD_NOT_NULL_CONSTRAINT"]) == "YES"), "auto_increment" => '0', "collation" => trim($K["FIELD_COLLATION"]), "privileges" => ["insert" => 1, "select" => 1, "update" => 1], "comment" => trim($K["FIELD_DESCRIPTION"]),];
        return $J;
    }
    ...
    support($lc) {
        return
            preg_match("~^(columns|sql|status|table)$~", $lc);
    }
...
$Jb["simpledb"] = "SimpleDB";
if (isset($_GET["simpledb"])) {
    $Me = ["SimpleXML + allow_url_fopen"];
    ...
    if (class_exists('SimpleXMLElement') && ini_bool('allow_url_fopen')) {
        ...
        Min_DB {
            ...
            function
            query($G, $Fg = false) {
                $E = ['SelectExpression' => $G, 'ConsistentRead' => 'true'];
                if ($this->next) $E['NextToken'] = $this->next;
                $I = sdb_request_all('Select', 'Item', $E, $this->timeout);
                $this->timeout = 0;
                if ($I === false) return $I;
                if (preg_match('~^\s*SELECT\s+COUNT\(~i', $G)) {
                    $Yf = 0;
                    foreach ($I
                             as $rd) $Yf += $rd->Attribute->Value;
                    $I = [(object)['Attribute' => [(object)['Name' => 'Count', 'Value' => $Yf,]]]];
                }
                return
                    new
                    Min_Result($I);
        ...
        Min_Result {
            ...
            __construct($I) {
                foreach ($I
                         as $rd) {
                    $K = [];
                    if ($rd->Name != '') $K['itemName()'] = (string)$rd->Name;
                    foreach ($rd->Attribute
                             as $Ca) {
                        $B = $this->_processValue($Ca->Name);
                        $Y = $this->_processValue($Ca->Value);
                        if (isset($K[$B])) {
                            $K[$B] = (array)$K[$B];
                            $K[$B][] = $Y;
                        } else$K[$B] = $Y;
            ...
            function
            _processValue($Qb) {
                return (is_object($Qb) && $Qb['encoding'] == 'base64' ? base64_decode($Qb) : (string)$Qb);
            }
            ...
            fetch_assoc() {
                ...
                if (!$K) return $K;
                $J = [];
                foreach ($this->_rows[0] as $y => $X) $J[$y] = $K[$y];
                next($this->_rows);
                return $J;
            }
            ...
            function
            fetch_field() {
                $wd = array_keys($this->_rows[0]);
                return (object)['name' => $wd[$this->_offset++]];
            }
        }
    }
    ...
    Min_Driver
        ...
        function
        _chunkRequest($Yc, $ra, $E, $dc = []) {
            global $h;
            foreach (array_chunk($Yc, 25) as $Za) {
                $Ce = $E;
                foreach ($Za
                         as $s => $t) {
                    $Ce["Item.$s.ItemName"] = $t;
                    foreach ($dc
                             as $y => $X) $Ce["Item.$s.$y"] = $X;
                }
                if (!sdb_request($ra, $Ce)) return
                    false;
            }
            $h->affected_rows = count($Yc);
            return
                true;
        }
        ...
        _extractIds($R, $H, $z) {
            $J = [];
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $H, $Pd)) $J = array_map('idf_unescape', $Pd[1]); else {
                foreach (sdb_request_all('Select', 'Item', ['SelectExpression' => 'SELECT itemName() FROM '.table($R).$H.($z ? " LIMIT 1" : "")]) as $rd) $J[] = $rd->Name;
            }
            return $J;
        }
        ...
        select($R, $M, $Z, $Hc, $ve = [], $z = 1, $D = 0, $Qe = false) {
            global $h;
            $h->next = $_GET["next"];
            $J = parent::select($R, $M, $Z, $Hc, $ve, $z, $D, $Qe);
            $h->next = 0;
            return $J;
        }
        ...
        function
        delete($R, $H, $z = 0) {
            return $this->_chunkRequest($this->_extractIds($R, $H, $z), 'BatchDeleteAttributes', ['DomainName' => $R]);
        }
        ...
        update($R, $P, $H, $z = 0, $N = "\n") {
            ...
            $ld = [];
            $s = 0;
            $Yc = $this->_extractIds($R, $H, $z);
            $t = idf_unescape($P["`itemName()`"]);
            unset($P["`itemName()`"]);
            foreach ($P
                     as $y => $X) {
                $y = idf_unescape($y);
                if ($X == "NULL" || ($t != "" && [$t] != $Yc)) $Bb["Attribute.".count($Bb).".Name"] = $y;
                if ($X != "NULL") {
                    foreach ((array)$X
                             as $sd => $W) {
                        $ld["Attribute.$s.Name"] = $y;
                        $ld["Attribute.$s.Value"] = (is_array($X) ? $W : idf_unescape($W));
                        if (!$sd) $ld["Attribute.$s.Replace"] = "true";
                        $s++;
                    }
                }
            }
            $E = ['DomainName' => $R];
            return (!$ld || $this->_chunkRequest(($t != "" ? [$t] : $Yc), 'BatchPutAttributes', $E, $ld)) && (!$Bb || $this->_chunkRequest($Yc, 'BatchDeleteAttributes', $E, $Bb));
        }
        ...
        insert($R, $P) {
            ...
            foreach ($P
                ...
                if ($Y != "NULL") {
                    $B = idf_unescape($B);
                    if ($B == "itemName()") $E["ItemName"] = idf_unescape($Y); else {
                        foreach ((array)$Y
                                 as $X) {
                            $E["Attribute.$s.Name"] = $B;
                            $E["Attribute.$s.Value"] = (is_array($Y) ? $X : idf_unescape($Y));
                            $s++;
                        }
                    }
                }
        ...
        insertUpdate($R, $L, $Oe) {
            foreach ($L
                     as $P) {
                if (!$this->update($R, $P, "WHERE `itemName()` = ".q($P["`itemName()`"]))) return
                    false;
            }
            return
                true;
    ...
    tables_list() {
        global $h;
        $J = [];
        foreach (sdb_request_all('ListDomains', 'DomainName') as $R) $J[(string)$R] = 'table';
    ...
    table_status($B = "", $kc = false) {
        $J = [];
        foreach (($B != "" ? [$B => true] : tables_list()) as $R => $U) {
            $K = ["Name" => $R, "Auto_increment" => ""];
            if (!$kc) {
                $Xd = sdb_request('DomainMetadata', ['DomainName' => $R]);
                if ($Xd) {
                    foreach (["Rows" => "ItemCount", "Data_length" => "ItemNamesSizeBytes", "Index_length" => "AttributeValuesSizeBytes", "Data_free" => "AttributeNamesSizeBytes",] as $y => $X) $K[$y] = (string)$Xd->$X;
                }
            }
            if ($B != "") return $K;
            $J[$R] = $K;
    ...
    function
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        return ($R == "" && sdb_request('CreateDomain', ['DomainName' => $B]));
    }
    ...
    drop_tables($T) {
        foreach ($T
                 as $R) {
            if (!sdb_request('DeleteDomain', ['DomainName' => $R])) return
                false;
        }
        return
            true;
    ...
    function
    hmac($wa, $xb, $y, $af = false) {
        $Pa = 64;
        if (strlen($y) > $Pa) $y = pack("H*", $wa($y));
        $y = str_pad($y, $Pa, "\0");
        $td = $y ^ str_repeat("\x36", $Pa);
        $ud = $y ^ str_repeat("\x5C", $Pa);
        $J = $wa($ud.pack("H*", $wa($td.$xb)));
        if ($af) $J = pack("H*", $J);
        return $J;
    }
    ...
    sdb_request($ra, $E = []) {
        global $b, $h;
        [$Uc, $E['AWSAccessKeyId'], $uf] = $b->credentials();
        $E['Action'] = $ra;
        $E['Timestamp'] = gmdate('Y-m-d\TH:i:s+00:00');
        $E['Version'] = '2009-04-15';
        $E['SignatureVersion'] = 2;
        $E['SignatureMethod'] = 'HmacSHA1';
        ksort($E);
        $G = '';
        foreach ($E
        ...
        $oc = @file_get_contents((preg_match('~^https?://~', $Uc) ? $Uc : "http://$Uc"), false, stream_context_create(['http' => ['method' => 'POST', 'content' => $G, 'ignore_errors' => 1,]]));
        if (!$oc) {
        ...
        $h->error = '';
        $dg = $ra."Result";
        return ($jh->$dg ? $jh->$dg : true);
    }

    function
    sdb_request_all($ra, $dg, $E = [], $lg = 0) {
        $J = [];
        $Qf = ($lg ? microtime(true) : 0);
        $z = (preg_match('~LIMIT\s+(\d+)\s*$~i', $E['SelectExpression'], $A) ? $A[1] : 0);
        do {
            $jh = sdb_request($ra, $E);
            if (!$jh) break;
            foreach ($jh->$dg
                     as $Qb) $J[] = $Qb;
            if ($z && count($J) >= $z) {
                $_GET["next"] = $jh->NextToken;
                break;
            }
            if ($lg && microtime(true) - $Qf > $lg) return
                false;
            $E['NextToken'] = $jh->NextToken;
            if ($z) $E['SelectExpression'] = preg_replace('~\d+\s*$~', $z - count($J), $E['SelectExpression']);
        } while ($jh->NextToken);
        return $J;
    }

    $x = "simpledb";
    $se = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "IS NOT NULL"];
    $Gc = [];
    $Kc = ["count"];
    $Ob = [["json"]];
}
...
if (isset($_GET["mongo"])) {
    ...
    if (class_exists('MongoDB')) {
        ...
        Min_Result {
            ...
            fetch_field() {
                $wd = array_keys($this->_rows[0]);
                $B = $wd[$this->_offset++];
                return (object)['name' => $B, 'charsetnr' => $this->_charset[$B],];
            }
        }
        ...
        Min_Driver
            ...
            select($R, $M, $Z, $Hc, $ve = [], $z = 1, $D = 0, $Qe = false) {
                ...
                foreach ($ve
                         as $X) {
                    $X = preg_replace('~ DESC$~', '', $X, 1, $rb);
                    $Jf[$X] = ($rb ? -1 : 1);
                }
                return
                    new
                    Min_Result($this->_conn->_db->selectCollection($R)->find([], $M)->sort($Jf)->limit($z != "" ? +$z : 0)->skip($D * $z));
            }

            function
            insert($R, $P) {
                try {
                    $J = $this->_conn->_db->selectCollection($R)->insert($P);
                    $this->_conn->errno = $J['code'];
                    $this->_conn->error = $J['err'];
                    $this->_conn->last_id = $P['_id'];
                    return !$J['err'];
                } catch (Exception$ac) {
        ...
        get_databases($vc) {
            ...
            $J = [];
            $zb = $h->_link->listDBs();
            foreach ($zb['databases'] as $m) $J[] = $m['name'];
            return $J;
        }
        ...
        count_tables($l) {
            ...
            $J = [];
            foreach ($l
                     as $m) $J[$m] = count($h->_link->selectDB($m)->getCollectionNames(true));
            return $J;
        }
        ...
        indexes($R, $i = null) {
            global $h;
            $J = [];
            foreach ($h->_db->selectCollection($R)->getIndexInfo() as $v) {
                $Eb = [];
                foreach ($v["key"] as $e => $U) $Eb[] = ($U == -1 ? '1' : null);
                $J[$v["name"]] = ["type" => ($v["name"] == "_id_" ? "PRIMARY" : ($v["unique"] ? "UNIQUE" : "INDEX")), "columns" => array_keys($v["key"]), "lengths" => [], "descs" => $Eb,];
            }
            return $J;
        }
            ...
            __construct($I) {
                foreach ($I
                    ...
                    foreach ($rd
                        ...
                        $K[$y] = (is_a($X, 'MongoDB\BSON\ObjectID') ? 'MongoDB\BSON\ObjectID("'.strval($X).'")' : (is_a($X, 'MongoDB\BSON\UTCDatetime') ? $X->toDateTime()->format('Y-m-d H:i:s') : (is_a($X, 'MongoDB\BSON\Binary') ? $X->bin : (is_a($X, 'MongoDB\BSON\Regex') ? strval($X) : (is_object($X) ? json_encode($X, 256) : $X)))));
                    }
                    $this->_rows[] = $K;
                    foreach ($K
                             as $y => $X) {
        ...
        Min_Driver
            ...
            select($R, $M, $Z, $Hc, $ve = [], $z = 1, $D = 0, $Qe = false) {
                global $h;
                $M = ($M == ["*"] ? [] : array_fill_keys($M, 1));
                if (count($M) && !isset($M['_id'])) $M['_id'] = 0;
                $Z = where_to_query($Z);
                $Jf = [];
                foreach ($ve
                         as $X) {
                ...
                $Gf = $D * $z;
                $bb = 'MongoDB\Driver\Query';
                $G = new$bb($Z, ['projection' => $M, 'limit' => $z, 'skip' => $Gf, 'sort' => $Jf]);
                $mf = $h->_link->executeQuery("$h->_db_name.$R", $G);
                return
                    new
                    Min_Result($mf);
            }
            ...
            update($R, $P, $H, $z = 0, $N = "\n") {
                global $h;
                $m = $h->_db_name;
                $Z = sql_query_where_parser($H);
                $bb = 'MongoDB\Driver\BulkWrite';
                $Ta = new$bb([]);
                if (isset($P['_id'])) unset($P['_id']);
                $ff = [];
                foreach ($P
                         as $y => $Y) {
                    if ($Y == 'NULL') {
                ...
                $Mg = ['$set' => $P];
                if (count($ff)) $Mg['$unset'] = $ff;
                $Ta->update($Z, $Mg, ['upsert' => false]);
                $mf = $h->_link->executeBulkWrite("$m.$R", $Ta);
                $h->affected_rows = $mf->getModifiedCount();
                return
                    true;
            }
            ...
            delete($R, $H, $z = 0) {
                ...
                $Ta->delete($Z, ['limit' => $z]);
                $mf = $h->_link->executeBulkWrite("$m.$R", $Ta);
                $h->affected_rows = $mf->getDeletedCount();
                return
                    true;
            }
            ...
            insert($R, $P) {
                ...
                $bb = 'MongoDB\Driver\BulkWrite';
                $Ta = new$bb([]);
                if (isset($P['_id']) && empty($P['_id'])) unset($P['_id']);
                $Ta->insert($P);
                $mf = $h->_link->executeBulkWrite("$m.$R", $Ta);
                $h->affected_rows = $mf->getInsertedCount();
                return
                    true;
            }
        }
        ...
        get_databases($vc) {
            global $h;
            $J = [];
            $bb = 'MongoDB\Driver\Command';
            $ib = new$bb(['listDatabases' => 1]);
            $mf = $h->_link->executeCommand('admin', $ib);
            foreach ($mf
                     as $zb) {
                foreach ($zb->databases
                         as $m) $J[] = $m->name;
            }
            return $J;
        }
        ...
        tables_list() {
            global $h;
            $bb = 'MongoDB\Driver\Command';
            $ib = new$bb(['listCollections' => 1]);
            $mf = $h->_link->executeCommand($h->_db_name, $ib);
            $gb = [];
            foreach ($mf
                     as $I) $gb[$I->name] = 'table';
            return $gb;
        }
        ...
        indexes($R, $i = null) {
            ...
            $J = [];
            $bb = 'MongoDB\Driver\Command';
            $ib = new$bb(['listIndexes' => $R]);
            $mf = $h->_link->executeCommand($h->_db_name, $ib);
            foreach ($mf
                     as $v) {
                $Eb = [];
                $f = [];
                foreach (get_object_vars($v->key) as $e => $U) {
                    $Eb[] = ($U == -1 ? '1' : null);
                    $f[] = $e;
                }
                $J[$v->name] = ["type" => ($v->name == "_id_" ? "PRIMARY" : (isset($v->unique) ? "UNIQUE" : "INDEX")), "columns" => $f, "lengths" => [], "descs" => $Eb,];
            }
            return $J;
        }
        ...
        fields($R) {
            ...
            if (!count($q)) {
                global $n;
                $I = $n->select($R, ["*"], null, null, [], 10);
                while ($K = $I->fetch_assoc()) {
                    foreach ($K
                             as $y => $X) {
                        $K[$y] = null;
                        $q[$y] = ["field" => $y, "type" => "string", "null" => ($y != $n->primary), "auto_increment" => ($y == $n->primary), "privileges" => ["insert" => 1, "select" => 1, "update" => 1,],];
                    }
                }
            }
            return $q;
        ...
        found_rows($S, $Z) {
            ...
            $Z = where_to_query($Z);
            $bb = 'MongoDB\Driver\Command';
            $ib = new$bb(['count' => $S['Name'], 'query' => $Z]);
            $mf = $h->_link->executeCommand($h->_db_name, $ib);
            $sg = $mf->toArray();
            return $sg[0]->n;
        }
        ...
        function
        sql_query_where_parser($H) {
            $H = trim(preg_replace('/WHERE[\s]?[(]?\(?/', '', $H));
            $H = preg_replace('/\)\)\)$/', ')', $H);
            $gh = explode(' AND ', $H);
            $hh = explode(') OR (', $H);
            $Z = [];
            foreach ($gh
                     as $eh) $Z[] = trim($eh);
            if (count($hh) == 1) $hh = []; elseif (count($hh) > 1) $Z = [];
            return
                where_to_query($Z, $hh);
        }

        function
        where_to_query($ch = [], $dh = []) {
            global $b;
            $xb = [];
            foreach (['and' => $ch, 'or' => $dh] as $U => $Z) {
                if (is_array($Z)) {
                    foreach ($Z
                             as $ec) {
                        [$eb, $qe, $X] = explode(" ", $ec, 3);
                        if ($eb == "_id") {
                            $X = str_replace('MongoDB\BSON\ObjectID("', "", $X);
                            $X = str_replace('")', "", $X);
                            $bb = 'MongoDB\BSON\ObjectID';
                            $X = new$bb($X);
                        }
                        if (!in_array($qe, $b->operators)) continue;
                        if (preg_match('~^\(f\)(.+)~', $qe, $A)) {
                            $X = (float)$X;
                            $qe = $A[1];
                        } elseif (preg_match('~^\(date\)(.+)~', $qe, $A)) {
                            $yb = new
                            DateTime($X);
                            $bb = 'MongoDB\BSON\UTCDatetime';
                            $X = new$bb($yb->getTimestamp() * 1000);
                            $qe = $A[1];
                        }
                        switch ($qe) {
                            case'=':
                        ...
                        if ($U == 'and') $xb['$and'][] = [$eb => [$qe => $X]]; elseif ($U == 'or') $xb['$or'][] = [$eb => [$qe => $X]];
                    }
                }
            }
            return $xb;
        ...
        $se = ["=", "!=", ">", "<", ">=", "<=", "regex", "(f)=", "(f)!=", "(f)>", "(f)<", "(f)>=", "(f)<=", "(date)=", "(date)!=", "(date)>", "(date)<", "(date)>=", "(date)<=",];
    }
    function
    table($u) {
        return $u;
    ...
    connect() {
        ...
        $m = $b->database();
        if ($m != "") $C["db"] = $m;
        if (($Fa = getenv("MONGO_AUTH_SOURCE"))) $C["authSource"] = $Fa;
        try {
            $h->_link = $h->connect("mongodb://$O", $C);
            if ($F != "") {
                $C["password"] = "";
                try {
                    $h->connect("mongodb://$O", $C);
                    return
                        lang(22);
                } catch (Exception$ac) {
                }
    ...
    alter_indexes($R, $c) {
        ...
        foreach ($c
                 as $X) {
            [$U, $B, $P] = $X;
            if ($P == "DROP") $J = $h->_db->command(["deleteIndexes" => $R, "index" => $B]); else {
                $f = [];
                foreach ($P
                         as $e) {
                    $e = preg_replace('~ DESC$~', '', $e, 1, $rb);
                    $f[$e] = ($rb ? -1 : 1);
                }
                $J = $h->_db->selectCollection($R)->ensureIndex($f, ["unique" => ($U == "UNIQUE"), "name" => $B,]);
            }
            if ($J['errmsg']) {
                $h->error = $J['errmsg'];
                return
    ...
    support($lc) {
        return
            preg_match("~database|indexes|descidx~", $lc);
    }
    ...
    drop_tables($T) {
        ...
        foreach ($T
                 as $R) {
            $jf = $h->_db->selectCollection($R)->drop();
            if (!$jf['ok']) return
                false;
        }
        return
    ...
    truncate_tables($T) {
        ...
        foreach ($T
                 as $R) {
            $jf = $h->_db->selectCollection($R)->remove();
            if (!$jf['ok']) return
                false;
        }
        return
...
$Jb["elastic"] = "Elasticsearch (beta)";
if (isset($_GET["elastic"])) {
    $Me = ["json + allow_url_fopen"];
    ...
    if (function_exists('json_decode') && ini_bool('allow_url_fopen')) {
        ...
        Min_DB {
            ...
            function
            rootQuery($Ge, $pb = [], $Yd = 'GET') {
                @ini_set('track_errors', 1);
                $oc = @file_get_contents("$this->_url/".ltrim($Ge, '/'), false, stream_context_create(['http' => ['method' => $Yd, 'content' => $pb === null ? $pb : json_encode($pb), 'header' => 'Content-Type: application/json', 'ignore_errors' => 1,]]));
                if (!$oc) {
            ...
            function
            query($Ge, $pb = [], $Yd = 'GET') {
                return $this->rootQuery(($this->_db != "" ? "$this->_db/" : "/").ltrim($Ge, '/'), $pb, $Yd);
            }

            function
            connect($O, $V, $F) {
                preg_match('~^(https?://)?(.*)~', $O, $A);
                $this->_url = ($A[1] ? $A[1] : "http://")."$V:$F@$A[2]";
                $J = $this->query('');
                if ($J) $this->server_info = $J['version']['number'];
                return (bool)$J;
            }
    ...
    Min_Driver
        ...
        select($R, $M, $Z, $Hc, $ve = [], $z = 1, $D = 0, $Qe = false) {
            ...
            foreach ($Z
                     as $X) {
                [$eb, $qe, $X] = explode(" ", $X, 3);
                if ($eb == "_id") $xb["query"]["ids"]["values"][] = $X; elseif ($eb.$X != "") {
                    $gg = ["term" => [($eb != "" ? $eb : "_all") => $X]];
                    if ($qe == "=") $xb["query"]["filtered"]["filter"]["and"][] = $gg; else$xb["query"]["filtered"]["query"]["bool"]["must"][] = $gg;
                }
            }
            if ($xb["query"] && !$xb["query"]["filtered"]["query"] && !$xb["query"]["ids"]) $xb["query"]["filtered"]["query"] = ["match_all" => []];
            $Qf = microtime(true);
            $tf = $this->_conn->query($G, $xb);
            if ($Qe) echo $b->selectQuery("$G: ".json_encode($xb), $Qf, !$tf);
            if (!$tf) return
                false;
            $J = [];
            foreach ($tf['hits']['hits'] as $Tc) {
                $K = [];
                if ($M == ["*"]) $K["_id"] = $Tc["_id"];
                $q = $Tc['_source'];
                if ($M != ["*"]) {
        ...
        update($U, $bf, $H, $z = 0, $N = "\n") {
            ...
            if (count($Fe) == 2) {
                $t = trim($Fe[1]);
                $G = "$U/$t";
                return $this->_conn->query($G, $bf, 'POST');
            }
            return
                false;
        }
        ...
        insert($U, $bf) {
            $t = "";
            $G = "$U/$t";
            $jf = $this->_conn->query($G, $bf, 'POST');
            $this->_conn->last_id = $jf['_id'];
            return $jf['created'];
        }
        ...
        delete($U, $H, $z = 0) {
            $Yc = [];
            if (is_array($_GET["where"]) && $_GET["where"]["_id"]) $Yc[] = $_GET["where"]["_id"];
            if (is_array($_POST['check'])) {
                foreach ($_POST['check'] as $Va) {
                    $Fe = preg_split('~ *= *~', $Va);
                    if (count($Fe) == 2) $Yc[] = trim($Fe[1]);
                }
            }
            ...
            foreach ($Yc
                     as $t) {
                $G = "{$U}/{$t}";
                $jf = $this->_conn->query($G, '{}', 'DELETE');
                if (is_array($jf) && $jf['found'] == true) $this->_conn->affected_rows++;
            }
    ...
    connect() {
        ...
        Min_DB;
        [$O, $V, $F] = $b->credentials();
        if ($F != "" && $h->connect($O, $V, "")) return
            lang(22);
        if ($h->connect($O, $V, $F)) return $h;
        return $h->error;
    }
    ...
    support($lc) {
        return
            preg_match("~database|table|columns~", $lc);
    }
    ...
    tables_list() {
        global $h;
        $J = $h->query('_mapping');
        if ($J) $J = array_fill_keys(array_keys($J[$h->_db]["mappings"]), 'table');
        return $J;
    }
    ...
    table_status($B = "", $kc = false) {
        global $h;
        $tf = $h->query("_search", ["size" => 0, "aggregations" => ["count_by_type" => ["terms" => ["field" => "_type"]]]], "POST");
        $J = [];
        if ($tf) {
            $T = $tf["aggregations"]["count_by_type"]["buckets"];
            foreach ($T
                     as $R) {
                $J[$R["key"]] = ["Name" => $R["key"], "Engine" => "table", "Rows" => $R["doc_count"],];
                if ($B != "" && $B == $R["key"]) return $J[$B];
            }
        }
        return $J;
    }
    ...
    fields($R) {
        global $h;
        $I = $h->query("$R/_mapping");
        $J = [];
        if ($I) {
            $Ld = $I[$R]['properties'];
            if (!$Ld) $Ld = $I[$h->_db]['mappings'][$R]['properties'];
            if ($Ld) {
                foreach ($Ld
                         as $B => $p) {
                    $J[$B] = ["field" => $B, "full_type" => $p["type"], "type" => $p["type"], "privileges" => ["insert" => 1, "select" => 1, "update" => 1],];
                    if ($p["properties"]) {
                        unset($J[$B]["privileges"]["insert"]);
                        unset($J[$B]["privileges"]["update"]);
                    }
                }
            }
        }
    ...
    drop_databases($l) {
        global $h;
        return $h->rootQuery(urlencode(implode(',', $l)), [], 'DELETE');
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        ...
        foreach ($q
                 as $ic) {
            $mc = trim($ic[1][0]);
            $nc = trim($ic[1][1] ? $ic[1][1] : "text");
            $Te[$mc] = ['type' => $nc];
        }
        if (!empty($Te)) $Te = ['properties' => $Te];
        return $h->query("_mapping/{$B}", $Te, 'PUT');
    }
    ...
    drop_tables($T) {
        ...
        $J = true;
        foreach ($T
                 as $R) $J = $J && $h->query(urlencode($R), [], 'DELETE');
        return $J;
    }
...
$Jb["clickhouse"] = "ClickHouse (alpha)";
if (isset($_GET["clickhouse"])) {
    ...
    Min_DB {
        ...
        rootQuery($m, $G) {
            @ini_set('track_errors', 1);
            $oc = @file_get_contents("$this->_url/?database=$m", false, stream_context_create(['http' => ['method' => 'POST', 'content' => $this->isQuerySelectLike($G) ? "$G FORMAT JSONCompact" : $G, 'header' => 'Content-type: application/x-www-form-urlencoded', 'ignore_errors' => 1,]]));
            if ($oc === false) {
                $this->error = $php_errormsg;
                return $oc;
            }
            ...
            $J = json_decode($oc, true);
            if ($J === null) {
                if (!$this->isQuerySelectLike($G) && $oc === '') return
                    true;
                $this->errno = json_last_error();
                if (function_exists('json_last_error_msg')) $this->error = json_last_error_msg(); else {
                    $ob = get_defined_constants(true);
    ...
    Min_Result {
        ...
        fetch_assoc() {
            $K = current($this->_rows);
            next($this->_rows);
            return $K === false ? false : array_combine($this->columns, $K);
        }
        ...
        fetch_field() {
            ...
            if ($e < count($this->columns)) {
                $J->name = $this->meta[$e]['name'];
                $J->orgname = $J->name;
                $J->type = $this->meta[$e]['type'];
            }
            return $J;
        }
    }
    ...
    Min_Driver
        ...
        delete($R, $H, $z = 0) {
            if ($H === '') $H = 'WHERE 1=1';
            return
                queries("ALTER TABLE ".table($R)." DELETE $H");
        }
        ...
        update($R, $P, $H, $z = 0, $N = "\n") {
            ...
            $G = $N.implode(",$N", $Ug);
            return
                queries("ALTER TABLE ".table($R)." UPDATE $G$H");
        }
    }
    ...
    function
    found_rows($S, $Z) {
        $L = get_vals("SELECT COUNT(*) FROM ".idf_escape($S["Name"]).($Z ? " WHERE ".implode(" AND ", $Z) : ""));
        return
            empty($L) ? false : $L[0];
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        ...
        foreach ($q
                 as $p) {
            if ($p[1][2] === " NULL") $p[1][1] = " Nullable({$p[1][1]})"; elseif ($p[1][2] === ' NOT NULL') $p[1][2] = '';
            if ($p[1][3]) $p[1][3] = '';
            $c[] = ($p[1] ? ($R != "" ? ($p[0] != "" ? "MODIFY COLUMN " : "ADD COLUMN ") : " ").implode($p[1]) : "DROP COLUMN ".idf_escape($p[0]));
            $ve[] = $p[1][0];
        }
        $c = array_merge($c, $xc);
        $Sf = ($Vb ? " ENGINE ".$Vb : "");
        if ($R == "") return
            queries("CREATE TABLE ".table($B)." (\n".implode(",\n", $c)."\n)$Sf$Ee".' ORDER BY ('.implode(',', $ve).')');
        if ($R != $B) {
            $I = queries("RENAME TABLE ".table($R)." TO ".table($B));
            if ($c) $R = $B; else
                return $I;
        }
        if ($Sf) $c[] = ltrim($Sf);
        return ($c || $Ee ? queries("ALTER TABLE ".table($R)."\n".implode(",\n", $c).$Ee) : true);
    }
    ...
    function
    limit($G, $Z, $z, $le = 0, $N = " ") {
        return " $G$Z".($z !== null ? $N."LIMIT $z".($le ? ", $le" : "") : "");
    }
    ...
    table_status($B = "", $kc = false) {
        global $h;
        $J = [];
        $T = get_rows("SELECT name, engine FROM system.tables WHERE database = ".q($h->_db));
        foreach ($T
                 as $R) {
            $J[$R['name']] = ['Name' => $R['name'], 'Engine' => $R['engine'],];
            if ($B === $R['name']) return $J[$R['name']];
        }
        return $J;
    }
    ...
    function
    unconvert_field($p, $J) {
        if (in_array($p['type'], ["Int8", "Int16", "Int32", "Int64", "UInt8", "UInt16", "UInt32", "UInt64", "Float32", "Float64"])) return "to$p[type]($J)";
        return $J;
    }
    ...
    fields($R) {
        $J = [];
        $I = get_rows("SELECT name, type, default_expression FROM system.columns WHERE ".idf_escape('table')." = ".q($R));
        foreach ($I
                 as $K) {
            $U = trim($K['type']);
            $he = strpos($U, 'Nullable(') === 0;
            $J[trim($K['name'])] = ["field" => trim($K['name']), "full_type" => $U, "type" => $U, "default" => trim($K['default_expression']), "null" => $he, "auto_increment" => '0', "privileges" => ["insert" => 1, "select" => 1, "update" => 0],];
        }
        return $J;
    }
    ...
    support($lc) {
        return
            preg_match("~^(columns|sql|status|table|drop_col)$~", $lc);
    }

    $x = "clickhouse";
    $Eg = [];
    $Uf = [];
    foreach ([lang(27) => ["Int8" => 3, "Int16" => 5, "Int32" => 10, "Int64" => 19, "UInt8" => 3, "UInt16" => 5, "UInt32" => 10, "UInt64" => 20, "Float32" => 7, "Float64" => 16, 'Decimal' => 38, 'Decimal32' => 9, 'Decimal64' => 18, 'Decimal128' => 38], lang(28) => ["Date" => 13, "DateTime" => 20], lang(25) => ["String" => 0], lang(29) => ["FixedString" => 0],] as $y => $X) {
        $Eg += $X;
        $Uf[$y] = array_keys($X);
    }
    $Lg = [];
...
if (!defined("DRIVER")) {
    ...
    if (extension_loaded("mysqli")) {
        ...
        Min_DB
            ...
            function
            connect($O = "", $V = "", $F = "", $k = null, $Ke = null, $If = null) {
                global $b;
                mysqli_report(MYSQLI_REPORT_OFF);
                [$Uc, $Ke] = explode(":", $O, 2);
                $Pf = $b->connectSsl();
                if ($Pf) $this->ssl_set($Pf['key'], $Pf['cert'], $Pf['ca'], '', '');
                $J = @$this->real_connect(($O != "" ? $Uc : ini_get("mysqli.default_host")), ($O.$V != "" ? $V : ini_get("mysqli.default_user")), ($O.$V.$F != "" ? $F : ini_get("mysqli.default_pw")), $k, (is_numeric($Ke) ? $Ke : ini_get("mysqli.default_port")), (!is_numeric($Ke) ? $Ke : $If), ($Pf ? 64 : 0));
            ...
            function
            set_charset($Ua) {
                if (parent::set_charset($Ua)) return
                    true;
        ...
        Min_DB {
            ...
            connect($O, $V, $F) {
                ...
                $this->_link = @mysql_connect(($O != "" ? $O : ini_get("mysql.default_host")), ("$O$V" != "" ? $V : ini_get("mysql.default_user")), ("$O$V$F" != "" ? $F : ini_get("mysql.default_password")), true, 131072);
            ...
            set_charset($Ua) {
                if (function_exists('mysql_set_charset')) {
                    if (mysql_set_charset($Ua, $this->_link)) return
                        true;
            ...
            function
            query($G, $Fg = false) {
                $I = @($Fg ? mysql_unbuffered_query($G, $this->_link) : mysql_query($G, $this->_link));
                $this->error = "";
                if (!$I) {
        ...
        Min_DB
            ...
            connect($O, $V, $F) {
                global $b;
                $C = [PDO::MYSQL_ATTR_LOCAL_INFILE => false];
                $Pf = $b->connectSsl();
                if ($Pf) {
                    if (!empty($Pf['key'])) $C[PDO::MYSQL_ATTR_SSL_KEY] = $Pf['key'];
                    if (!empty($Pf['cert'])) $C[PDO::MYSQL_ATTR_SSL_CERT] = $Pf['cert'];
                    if (!empty($Pf['ca'])) $C[PDO::MYSQL_ATTR_SSL_CA] = $Pf['ca'];
                }
                $this->dsn("mysql:charset=utf8;host=".str_replace(":", ";unix_socket=", preg_replace('~:(\d)~', ';port=\1', $O)), $V, $F, $C);
                return
                    true;
            }
    ...
    Min_Driver
        ...
        function
        insert($R, $P) {
            return ($P ? parent::insert($R, $P) : queries("INSERT INTO ".table($R)." ()\nVALUES ()"));
        }

        function
        insertUpdate($R, $L, $Oe) {
            $f = array_keys(reset($L));
            $Ne = "INSERT INTO ".table($R)." (".implode(", ", $f).") VALUES\n";
            $Ug = [];
            foreach ($f
                     as $y) $Ug[$y] = "$y = VALUES($y)";
            ...
            foreach ($L
                     as $P) {
                $Y = "(".implode(", ", $P).")";
                if ($Ug && (strlen($Ne) + $Ed + strlen($Y) + strlen($Xf) > 1e6)) {
                    if (!queries($Ne.implode(",\n", $Ug).$Xf)) return
                        false;
                    $Ug = [];
                    $Ed = 0;
                }
        ...
        slowQuery($G, $lg) {
            if (min_version('5.7.8', '10.1.2')) {
                if (preg_match('~MariaDB~', $this->_conn->server_info)) return "SET STATEMENT max_statement_time=$lg FOR $G"; elseif (preg_match('~^(SELECT\b)(.+)~is', $G, $A)) return "$A[1] /*+ MAX_EXECUTION_TIME(".($lg * 1000).") */ $A[2]";
            }
        }

        function
        convertSearch($u, $X, $p) {
            return (preg_match('~char|text|enum|set~', $p["type"]) && !preg_match("~^utf8~", $p["collation"]) && preg_match('~[\x80-\xFF]~', $X['val']) ? "CONVERT($u USING ".charset($this->_conn).")" : $u);
        }

        function
        warnings() {
            $I = $this->_conn->query("SHOW WARNINGS");
            if ($I && $I->num_rows) {
                ob_start();
                select($I);
                return
        ...
        function
        tableHelp($B) {
            $Md = preg_match('~MariaDB~', $this->_conn->server_info);
            if (information_schema(DB)) return
                strtolower(($Md ? "information-schema-$B-table/" : str_replace("_", "-", $B)."-table.html"));
            if (DB == "mysql") return ($Md ? "mysql$B-table/" : "system-database.html");
        }
    }
    ...
    connect() {
        ...
        $J = $h->error;
        if (function_exists('iconv') && !is_utf8($J) && strlen($qf = iconv("windows-1250", "utf-8", $J)) > strlen($J)) $J = $qf;
        return $J;
    }
    ...
    db_collation($m, $fb) {
        global $h;
        $J = null;
        $sb = $h->result("SHOW CREATE DATABASE ".idf_escape($m), 1);
        if (preg_match('~ COLLATE ([^ ]+)~', $sb, $A)) $J = $A[1]; elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $sb, $A)) $J = $fb[$A[1]][-1];
        return $J;
    }
    ...
    count_tables($l) {
        $J = [];
        foreach ($l
                 as $m) $J[$m] = count(get_vals("SHOW TABLES IN ".idf_escape($m)));
        return $J;
    }
    ...
    table_status($B = "", $kc = false) {
        $J = [];
        foreach (get_rows($kc && min_version(5) ? "SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ".($B != "" ? "AND TABLE_NAME = ".q($B) : "ORDER BY Name") : "SHOW TABLE STATUS".($B != "" ? " LIKE ".q(addcslashes($B, "%_\\")) : "")) as $K) {
    ...
    indexes($R, $i = null) {
        $J = [];
        foreach (get_rows("SHOW INDEX FROM ".table($R), $i) as $K) {
            $B = $K["Key_name"];
            $J[$B]["type"] = ($B == "PRIMARY" ? "PRIMARY" : ($K["Index_type"] == "FULLTEXT" ? "FULLTEXT" : ($K["Non_unique"] ? ($K["Index_type"] == "SPATIAL" ? "SPATIAL" : "INDEX") : "UNIQUE")));
            $J[$B]["columns"][] = $K["Column_name"];
            $J[$B]["lengths"][] = ($K["Index_type"] == "SPATIAL" ? null : $K["Sub_part"]);
            $J[$B]["descs"][] = null;
        }
        return $J;
    }
    ...
    foreign_keys($R) {
        ...
        static $He = '(?:`(?:[^`]|``)+`|"(?:[^"]|"")+")';
        $J = [];
        $tb = $h->result("SHOW CREATE TABLE ".table($R), 1);
        if ($tb) {
            preg_match_all("~CONSTRAINT ($He) FOREIGN KEY ?\\(((?:$He,? ?)+)\\) REFERENCES ($He)(?:\\.($He))? \\(((?:$He,? ?)+)\\)(?: ON DELETE ($ne))?(?: ON UPDATE ($ne))?~", $tb, $Pd, PREG_SET_ORDER);
            foreach ($Pd
                     as $A) {
                preg_match_all("~$He~", $A[2], $Kf);
                preg_match_all("~$He~", $A[5], $eg);
                $J[idf_unescape($A[1])] = ["db" => idf_unescape($A[4] != "" ? $A[3] : $A[4]), "table" => idf_unescape($A[4] != "" ? $A[4] : $A[3]), "source" => array_map('idf_unescape', $Kf[0]), "target" => array_map('idf_unescape', $eg[0]), "on_delete" => ($A[6] ? $A[6] : "RESTRICT"), "on_update" => ($A[7] ? $A[7] : "RESTRICT"),];
            }
        }
        return $J;
    }
    ...
    view($B) {
        global $h;
        return
            ["select" => preg_replace('~^(?:[^`]|`[^`]*`)*\s+AS\s+~isU', '', $h->result("SHOW CREATE VIEW ".table($B), 1))];
    }
    ...
    collations() {
        ...
        foreach (get_rows("SHOW COLLATION") as $K) {
            if ($K["Default"]) $J[$K["Charset"]][-1] = $K["Collation"]; else$J[$K["Charset"]][] = $K["Collation"];
        }
        ksort($J);
        foreach ($J
                 as $y => $X) asort($J[$y]);
    ...
    rename_database($B, $d) {
        ...
        if (create_database($B, $d)) {
            $gf = [];
            foreach (tables_list() as $R => $U) $gf[] = table($R)." TO ".idf_escape($B).".".table($R);
            $J = (!$gf || queries("RENAME TABLE ".implode(", ", $gf)));
            if ($J) queries("DROP DATABASE ".idf_escape(DB));
            restart_session();
            set_session("dbs", null);
        }
        return $J;
    ...
    auto_increment() {
        ...
        if ($_GET["create"] != "" && $_POST["auto_increment_col"]) {
            foreach (indexes($_GET["create"]) as $v) {
                ...
                if ($v["type"] == "PRIMARY") $Ha = " UNIQUE";
            }
        }
        return " AUTO_INCREMENT$Ha";
    }
    ...
    alter_table($R, $B, $q, $xc, $jb, $Vb, $d, $Ga, $Ee) {
        $c = [];
        foreach ($q
                 as $p) $c[] = ($p[1] ? ($R != "" ? ($p[0] != "" ? "CHANGE ".idf_escape($p[0]) : "ADD") : " ")." ".implode($p[1]).($R != "" ? $p[2] : "") : "DROP ".idf_escape($p[0]));
        $c = array_merge($c, $xc);
        $Sf = ($jb !== null ? " COMMENT=".q($jb) : "").($Vb ? " ENGINE=".q($Vb) : "").($d ? " COLLATE ".q($d) : "").($Ga != "" ? " AUTO_INCREMENT=$Ga" : "");
        if ($R == "") return
            queries("CREATE TABLE ".table($B)." (\n".implode(",\n", $c)."\n)$Sf$Ee");
        if ($R != $B) $c[] = "RENAME TO ".table($B);
        if ($Sf) $c[] = ltrim($Sf);
        return ($c || $Ee ? queries("ALTER TABLE ".table($R)."\n".implode(",\n", $c).$Ee) : true);
    }
    ...
    move_tables($T, $Yg, $eg) {
        $gf = [];
        foreach (array_merge($T, $Yg) as $R) $gf[] = table($R)." TO ".idf_escape($eg).".".table($R);
        return
            queries("RENAME TABLE ".implode(", ", $gf));
    }
    ...
    copy_tables($T, $Yg, $eg) {
        ...
        foreach ($T
                 as $R) {
            $B = ($eg == DB ? table("copy_$R") : idf_escape($eg).".".table($R));
            ...
            foreach (get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($R, "%_\\"))) as $K) {
                $_g = $K["Trigger"];
                if (!queries("CREATE TRIGGER ".($eg == DB ? idf_escape("copy_$_g") : idf_escape($eg).".".idf_escape($_g))." $K[Timing] $K[Event] ON $B FOR EACH ROW\n$K[Statement];")) return
                    false;
            }
        }
        foreach ($Yg
            ...
            $B = ($eg == DB ? table("copy_$R") : idf_escape($eg).".".table($R));
            $Xg = view($R);
            if (($_POST["overwrite"] && !queries("DROP VIEW IF EXISTS $B")) || !queries("CREATE VIEW $B AS $Xg[select]")) return
                false;
        }
        return
            true;
    ...
    trigger($B) {
        if ($B == "") return
            [];
        $L = get_rows("SHOW TRIGGERS WHERE `Trigger` = ".q($B));
        return
            reset($L);
    }
    ...
    triggers($R) {
        $J = [];
        foreach (get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($R, "%_\\"))) as $K) $J[$K["Trigger"]] = [$K["Timing"], $K["Event"]];
        return $J;
    }
    ...
    routine($B, $U) {
        global $h, $Wb, $jd, $Eg;
        $xa = ["bool", "boolean", "integer", "double precision", "real", "dec", "numeric", "fixed", "national char", "national varchar"];
        $Lf = "(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";
        $Dg = "((".implode("|", array_merge(array_keys($Eg), $xa)).")\\b(?:\\s*\\(((?:[^'\")]|$Wb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";
        $He = "$Lf*(".($U == "FUNCTION" ? "" : $jd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Dg";
        $sb = $h->result("SHOW CREATE $U ".idf_escape($B), 2);
        preg_match("~\\(((?:$He\\s*,?)*)\\)\\s*".($U == "FUNCTION" ? "RETURNS\\s+$Dg\\s+" : "")."(.*)~is", $sb, $A);
        $q = [];
        preg_match_all("~$He\\s*,?~is", $A[1], $Pd, PREG_SET_ORDER);
        foreach ($Pd
                 as $Be) $q[] = ["field" => str_replace("``", "`", $Be[2]).$Be[3], "type" => strtolower($Be[5]), "length" => preg_replace_callback("~$Wb~s", 'normalize_enum', $Be[6]), "unsigned" => strtolower(preg_replace('~\s+~', ' ', trim("$Be[8] $Be[7]"))), "null" => 1, "full_type" => $Be[4], "inout" => strtoupper($Be[1]), "collation" => strtolower($Be[9]),];
        if ($U != "FUNCTION") return
            ["fields" => $q, "definition" => $A[11]];
        return
            ["fields" => $q, "returns" => ["type" => $A[12], "length" => $A[13], "unsigned" => $A[15], "collation" => $A[16]], "definition" => $A[17], "language" => "SQL",];
    }
    ...
    create_sql($R, $Ga, $Vf) {
        global $h;
        $J = $h->result("SHOW CREATE TABLE ".table($R), 1);
        if (!$Ga) $J = preg_replace('~ AUTO_INCREMENT=\d+~', '', $J);
        return $J;
    }
    ...
    function
    unconvert_field($p, $J) {
        if (preg_match("~binary~", $p["type"])) $J = "UNHEX($J)";
        if ($p["type"] == "bit") $J = "CONV($J, 2, 10) + 0";
        if (preg_match("~geometry|point|linestring|polygon~", $p["type"])) $J = (min_version(8) ? "ST_" : "")."GeomFromText($J, SRID($p[field]))";
        return $J;
    }

    function
    support($lc) {
        return !preg_match("~scheme|sequence|type|view_trigger|materializedview".(min_version(8) ? "" : "|descidx".(min_version(5.1) ? "" : "|event|partitioning".(min_version(5) ? "" : "|routine|trigger|view")))."~", $lc);
    }
    ...
    $Lg = ["unsigned", "zerofill", "unsigned zerofill"];
    $se = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "REGEXP", "IN", "FIND_IN_SET", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL"];
    $Gc = ["char_length", "date", "from_unixtime", "lower", "round", "floor", "ceil", "sec_to_time", "time_to_sec", "upper"];
    $Kc = ["avg", "count", "count distinct", "group_concat", "max", "min", "sum"];
    $Ob = [["char" => "md5/sha1/password/encrypt/uuid", "binary" => "md5/sha1", "date|time" => "now",], [number_type() => "+/-", "date" => "+ interval/- interval", "time" => "addtime/subtime", "char|text" => "concat",]];
}
...
Adminer {
    ...
    function
    name() {
        return "<a href='https://www.adminer.org/editor/'".target_blank()." id='h1'>".lang(34)."</a>";
    }
    ...
    database() {
        global $h;
        if ($h) {
            $l = $this->databases(false);
            return (!$l ? $h->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1)") : $l[(information_schema($l[0]) ? 1 : 0)]);
        }
    }
    ...
    tableName($ag) {
        return
            h($ag["Comment"] != "" ? $ag["Comment"] : $ag["Name"]);
    }

    function
    fieldName($p, $ve = 0) {
        return
            h(preg_replace('~\s+\[.*\]$~', '', ($p["comment"] != "" ? $p["comment"] : $p["field"])));
    }

    function
    selectLinks($ag, $P = "") {
        $a = $ag["Name"];
        if ($P !== null) echo '<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.lang(39)."</a>\n";
    }
...
ORDER BY ORDINAL_POSITION", null, "") as $K) $J[$K["TABLE_NAME"]]["keys"][$K["CONSTRAINT_NAME"]][$K["COLUMN_NAME"]] = $K["REFERENCED_COLUMN_NAME"];
        foreach ($J
                 as $y => $X) {
            $B = $this->tableName(table_status($y, true));
            if ($B != "") {
                $tf = preg_quote($Zf);
                $N = "(:|\\s*-)?\\s+";
                $J[$y]["name"] = (preg_match("(^$tf$N(.+)|^(.+?)$N$tf\$)iu", $B, $A) ? $A[2].$A[3] : $B);
            } else
                unset($J[$y]);
        }
        return $J;
    ...
    function
    backwardKeysPrint($Ka, $K) {
        foreach ($Ka
                 as $R => $Ja) {
            foreach ($Ja["keys"] as $hb) {
                $_ = ME.'select='.urlencode($R);
                $s = 0;
                foreach ($hb
                         as $e => $X) $_ .= where_link($s++, $e, $K[$X]);
                echo "<a href='".h($_)."'>".h($Ja["name"])."</a>";
                $_ = ME.'edit='.urlencode($R);
                foreach ($hb
                         as $e => $X) $_ .= "&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($K[$X]);
                echo "<a href='".h($_)."' title='".lang(39)."'>+</a> ";
            }
        }
    }

    function
    selectQuery($G, $Qf, $jc = false) {
        return "<!--\n".str_replace("--", "--><!-- ", $G)."\n(".format_time($Qf).")\n-->\n";
    }

    function
    rowDescription($R) {
        foreach (fields($R) as $p) {
            if (preg_match("~varchar|character varying~", $p["type"])) return
                idf_escape($p["field"]);
        }
        return "";
    }
    ...
    function
    rowDescriptions($L, $zc) {
        $J = $L;
        foreach ($L[0] as $y => $X) {
            if (list($R, $t, $B) = $this->_foreignColumn($zc, $y)) {
                $Yc = [];
                foreach ($L
                         as $K) $Yc[$K[$y]] = q($K[$y]);
                $Db = $this->_values[$R];
                if (!$Db) $Db = get_key_vals("SELECT $t, $B FROM ".table($R)." WHERE $t IN (".implode(", ", $Yc).")");
                foreach ($L
                         as $ce => $K) {
                    if (isset($K[$y])) $J[$ce][$y] = (string)$Db[$K[$y]];
                }
            }
        }
        return $J;
    ...
    selectVal($X, $_, $p, $xe) {
        $J = $X;
        $_ = h($_);
        if (preg_match('~blob|bytea~', $p["type"]) && !is_utf8($X)) {
            $J = lang(40, strlen($xe));
            if (preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~", $xe)) $J = "<img src='$_' alt='$J'>";
        }
        if (like_bool($p) && $J != "") $J = (preg_match('~^(1|t|true|y|yes|on)$~i', $X) ? lang(41) : lang(42));
        if ($_) $J = "<a href='$_'".(is_url($_) ? target_blank() : "").">$J</a>";
        if (!$_ && !like_bool($p) && preg_match(number_type(), $p["type"])) $J = "<div class='number'>$J</div>"; elseif (preg_match('~date~', $p["type"])) $J = "<div class='datetime'>$J</div>";
        return $J;
    }

    function
    editVal($X, $p) {
        if (preg_match('~date|timestamp~', $p["type"]) && $X !== null) return
            preg_replace('~^(\d{2}(\d+))-(0?(\d+))-(0?(\d+))~', lang(43), $X);
        return $X;
    }

    function
    selectColumnsPrint($M, $f) {
    }

    function
    selectSearchPrint($Z, $f, $w) {
        $Z = (array)$_GET["where"];
        echo '<fieldset id="fieldset-search"><legend>'.lang(44)."</legend><div>\n";
        $wd = [];
        foreach ($Z
                 as $y => $X) $wd[$X["col"]] = $y;
        $s = 0;
        $q = fields($_GET["select"]);
        foreach ($f
                 as $B => $Cb) {
            $p = $q[$B];
            if (preg_match("~enum~", $p["type"]) || like_bool($p)) {
                $y = $wd[$B];
                $s--;
                echo "<div>".h($Cb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'>:", (like_bool($p) ? " <select name='where[$s][val]'>".optionlist(["" => "", lang(42), lang(41)], $Z[$y]["val"], true)."</select>" : enum_input("checkbox", " name='where[$s][val][]'", $p, (array)$Z[$y]["val"], ($p["null"] ? 0 : null))), "</div>\n";
                unset($f[$B]);
            } elseif (is_array($C = $this->_foreignKeyOptions($_GET["select"], $B))) {
                if ($q[$B]["null"]) $C[0] = '('.lang(7).')';
                $y = $wd[$B];
                $s--;
                echo "<div>".h($Cb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($C, $Z[$y]["val"], true)."</select></div>\n";
                unset($f[$B]);
            }
        }
        $s = 0;
        foreach ($Z
                 as $X) {
            if (($X["col"] == "" || $f[$X["col"]]) && "$X[col]$X[val]" != "") {
                echo "<div><select name='where[$s][col]'><option value=''>(".lang(45).")".optionlist($f, $X["col"], true)."</select>", html_select("where[$s][op]", [-1 => ""] + $this->operators, $X["op"]), "<input type='search' name='where[$s][val]' value='".h($X["val"])."'>".script("mixin(qsl('input'), {onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});", "")."</div>\n";
                $s++;
            }
        }
        echo "<div><select name='where[$s][col]'><option value=''>(".lang(45).")".optionlist($f, null, true)."</select>", script("qsl('select').onchange = selectAddRow;", ""), html_select("where[$s][op]", [-1 => ""] + $this->operators), "<input type='search' name='where[$s][val]'></div>", script("mixin(qsl('input'), {onchange: function () { this.parentNode.firstChild.onchange(); }, onsearch: selectSearchSearch});"), "</div></fieldset>\n";
    }

    function
    selectOrderPrint($ve, $f, $w) {
        $we = [];
        foreach ($w
                 as $y => $v) {
            $ve = [];
            foreach ($v["columns"] as $X) $ve[] = $f[$X];
            if (count(array_filter($ve, 'strlen')) > 1 && $y != "PRIMARY") $we[$y] = implode(", ", $ve);
        }
        if ($we) {
            echo '<fieldset><legend>'.lang(46)."</legend><div>", "<select name='index_order'>".optionlist(["" => ""] + $we, ($_GET["order"][0] != "" ? "" : $_GET["index_order"]), true)."</select>", "</div></fieldset>\n";
        }
        if ($_GET["order"]) echo "<div style='display: none;'>".hidden_fields(["order" => [1 => reset($_GET["order"])], "desc" => ($_GET["desc"] ? [1 => 1] : []),])."</div>\n";
    }
    ...
    selectLimitPrint($z) {
        ...
        echo
        html_select("limit", ["", "50", "100"], $z), "</div></fieldset>\n";
    }

    function
    selectLengthPrint($ig) {
    }

    function
    selectActionPrint($w) {
        echo "<fieldset><legend>".lang(48)."</legend><div>", "<input type='submit' value='".lang(49)."'>", "</div></fieldset>\n";
    }
    ...
    selectSearchProcess($q, $w) {
        ...
        foreach ((array)$_GET["where"] as $y => $Z) {
            ...
            if (($y < 0 ? "" : $eb).$X != "") {
                $lb = [];
                foreach (($eb != "" ? [$eb => $q[$eb]] : $q) as $B => $p) {
                    if ($eb != "" || is_numeric($X) || !preg_match(number_type(), $p["type"])) {
                        $B = idf_escape($B);
                        if ($eb != "" && $p["type"] == "enum") $lb[] = (in_array(0, $X) ? "$B IS NULL OR " : "")."$B IN (".implode(", ", array_map('intval', $X)).")"; else {
                            $jg = preg_match('~char|text|enum|set~', $p["type"]);
                            $Y = $this->processInput($p, (!$qe && $jg && preg_match('~^[^%]+$~', $X) ? "%$X%" : $X));
                            $lb[] = $B.($Y == "NULL" ? " IS".($qe == ">=" ? " NOT" : "")." $Y" : (in_array($qe, $this->operators) || $qe == "=" ? " $qe $Y" : ($jg ? " LIKE $Y" : " IN (".str_replace(",", "', '", $Y).")")));
                            if ($y < 0 && $X == "0") $lb[] = "$B IS NULL";
                        }
                    }
                }
                $J[] = ($lb ? "(".implode(" OR ", $lb).")" : "1 = 0");
            }
        }
        return $J;
    }
    ...
    function
    selectOrderProcess($q, $w) {
        $bd = $_GET["index_order"];
        if ($bd != "") unset($_GET["order"][1]);
        if ($_GET["order"]) return
            [idf_escape(reset($_GET["order"])).($_GET["desc"] ? " DESC" : "")];
        foreach (($bd != "" ? [$w[$bd]] : $w) as $v) {
            if ($bd != "" || $v["type"] == "INDEX") {
                $Mc = array_filter($v["descs"]);
                $Cb = false;
                foreach ($v["columns"] as $X) {
                    if (preg_match('~date|timestamp~', $q[$X]["type"])) {
                        $Cb = true;
                        break;
                    }
                }
                $J = [];
                foreach ($v["columns"] as $y => $X) $J[] = idf_escape($X).(($Mc ? $v["descs"][$y] : $Cb) ? " DESC" : "");
                return $J;
            }
        }
        return
    ...
    function
    selectEmailProcess($Z, $zc) {
        if ($_POST["email_append"]) return
            true;
        if ($_POST["email"]) {
            $yf = 0;
            if ($_POST["all"] || $_POST["check"]) {
                $p = idf_escape($_POST["email_field"]);
                $Wf = $_POST["email_subject"];
                $Vd = $_POST["email_message"];
                preg_match_all('~\{\$([a-z0-9_]+)\}~i', "$Wf.$Vd", $Pd);
                $L = get_rows("SELECT DISTINCT $p".($Pd[1] ? ", ".implode(", ", array_map('idf_escape', array_unique($Pd[1]))) : "")." FROM ".table($_GET["select"])." WHERE $p IS NOT NULL AND $p != ''".($Z ? " AND ".implode(" AND ", $Z) : "").($_POST["all"] ? "" : " AND ((".implode(") OR (", array_map('where_check', (array)$_POST["check"]))."))"));
                $q = fields($_GET["select"]);
                foreach ($this->rowDescriptions($L, $zc) as $K) {
                    $hf = ['{\\' => '{'];
                    foreach ($Pd[1] as $X) $hf['{$'."$X}"] = $this->editVal($K[$X], $q[$X]);
                    $Rb = $K[$_POST["email_field"]];
                    if (is_mail($Rb) && send_mail($Rb, strtr($Wf, $hf), strtr($Vd, $hf), $_POST["email_from"], $_FILES["email_files"])) $yf++;
                }
            }
            cookie("adminer_email", $_POST["email_from"]);
            redirect(remove_from_uri(), lang(55, $yf));
        }
        return
            false;
    }
    ...
    function
    selectQueryBuild($M, $Z, $Hc, $ve, $z, $D) {
        return "";
    }

    function
    messageQuery($G, $kg, $jc = false) {
    ...
    editFunctions($p) {
        $J = [];
        if ($p["null"] && preg_match('~blob~', $p["type"])) $J["NULL"] = lang(7);
        $J[""] = ($p["null"] || $p["auto_increment"] || like_bool($p) ? "" : "*");
        if (preg_match('~date|time~', $p["type"])) $J["now"] = lang(56);
        if (preg_match('~_(md5|sha1)$~i', $p["field"], $A)) $J[] = strtolower($A[1]);
        return $J;
    }

    function
    editInput($R, $p, $Da, $Y) {
        if ($p["type"] == "enum") return (isset($_GET["select"]) ? "<label><input type='radio'$Da value='-1' checked><i>".lang(8)."</i></label> " : "").enum_input("radio", $Da, $p, ($Y || isset($_GET["select"]) ? $Y : 0), ($p["null"] ? "" : null));
        $C = $this->_foreignKeyOptions($R, $p["field"], $Y);
        if ($C !== null) return (is_array($C) ? "<select$Da>".optionlist($C, $Y, true)."</select>" : "<input value='".h($Y)."'$Da class='hidden'>"."<input value='".h($C)."' class='jsonly'>"."<div></div>".script("qsl('input').oninput = partial(whisper, '".ME."script=complete&source=".urlencode($R)."&field=".urlencode($p["field"])."&value=');
qsl('div').onclick = whisperClick;", ""));
        if (like_bool($p)) return '<input type="checkbox" value="1"'.(preg_match('~^(1|t|true|y|yes|on)$~i', $Y) ? ' checked' : '')."$Da>";
        $Sc = "";
        if (preg_match('~time~', $p["type"])) $Sc = lang(57);
        if (preg_match('~date|timestamp~', $p["type"])) $Sc = lang(58).($Sc ? " [$Sc]" : "");
        if ($Sc) return "<input value='".h($Y)."'$Da> ($Sc)";
        if (preg_match('~_(md5|sha1)$~i', $p["field"])) return "<input type='password' value='".h($Y)."'$Da>";
        return '';
    }

    function
    editHint($R, $p, $Y) {
        return (preg_match('~\s+(\[.*\])$~', ($p["comment"] != "" ? $p["comment"] : $p["field"]), $A) ? h(" $A[1]") : '');
    }
    ...
    processInput($p, $Y, $Fc = "") {
        if ($Fc == "now") return "$Fc()";
        $J = $Y;
        if (preg_match('~date|timestamp~', $p["type"]) && preg_match('(^'.str_replace('\$1', '(?P<p1>\d*)', preg_replace('~(\\\\\\$([2-6]))~', '(?P<p\2>\d{1,2})', preg_quote(lang(43)))).'(.*))', $Y, $A)) $J = ($A["p1"] != "" ? $A["p1"] : ($A["p2"] != "" ? ($A["p2"] < 70 ? 20 : 19).$A["p2"] : gmdate("Y")))."-$A[p3]$A[p4]-$A[p5]$A[p6]".end($A);
        $J = ($p["type"] == "bit" && preg_match('~^[0-9]+$~', $Y) ? $J : q($J));
        if ($Y == "" && like_bool($p)) $J = "'0'"; elseif ($Y == "" && ($p["null"] || !preg_match('~char|text~', $p["type"]))) $J = "NULL";
		elseif (preg_match('~^(md5|sha1)$~', $Fc)) $J = "$Fc($J)";
        return
            unconvert_field($p, $J);
    }
...
';
        if ($Zd == "auth") {
            $rc = true;
            foreach ((array)$_SESSION["pwds"] as $Vg => $Cf) {
                foreach ($Cf[""] as $V => $F) {
                    if ($F !== null) {
                        if ($rc) {
                        ...
                        echo "<li><a href='".h(auth_url($Vg, "", $V))."'>".($V != "" ? h($V) : "<i>".lang(7)."</i>")."</a>\n";
                    }
                }
            }
        } else {
            $this->databasesPrint($Zd);
            if ($Zd != "db" && $Zd != "ns") {
                $S = table_status('', true);
    ...
    tablesPrint($T) {
        ...
        foreach ($T
            ...
            echo '<li>';
            $B = $this->tableName($K);
            if (isset($K["Engine"]) && $B != "") echo "<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"] == $K["Name"] || $_GET["edit"] == $K["Name"], "select")." title='".lang(59)."'>$B</a>\n";
        }
        echo "</ul>\n";
    }
    ...
    function
    _foreignColumn($zc, $e) {
        foreach ((array)$zc[$e] as $yc) {
            if (count($yc["source"]) == 1) {
                $B = $this->rowDescription($yc["table"]);
                if ($B != "") {
                    $t = idf_escape($yc["target"][0]);
                    return
                        [$yc["table"], $t, $B];
                }
            }
    ...
    function
    _foreignKeyOptions($R, $e, $Y = null) {
        global $h;
        if (list($eg, $t, $B) = $this->_foreignColumn(column_foreign_keys($R), $e)) {
            $J =& $this->_values[$eg];
            if ($J === null) {
                $S = table_status($eg);
                $J = ($S["Rows"] > 1000 ? "" : ["" => ""] + get_key_vals("SELECT $t, $B FROM ".table($eg)." ORDER BY 2"));
            }
            if (!$J && $Y !== null) return $h->result("SELECT $B FROM ".table($eg)." WHERE $t = ".q($Y));
            return $J;
        }
    }
}
...
';
        foreach ($b->css() as $vb) {
            echo '<link rel="stylesheet" type="text/css" href="', h($vb), '">
';
        }
    }
    echo '
...
';
    $r = get_temp_dir()."/adminer.version";
    if (!$_COOKIE["adminer_version"] && function_exists('openssl_verify') && file_exists($r) && filemtime($r) + 86400 > time()) {
        $Wg = unserialize(file_get_contents($r));
        $Ue = "-----BEGIN PUBLIC KEY-----
...
-----END PUBLIC KEY-----
";
        if (openssl_verify($Wg["version"], base64_decode($Wg["signature"]), $Ue) == 1) $_COOKIE["adminer_version"] = $Wg["version"];
    }
    echo '<script', nonce(), '>
...
';
    if ($Sa !== null) {
        ...
        echo '<p id="breadcrumb"><a href="'.h($_ ? $_ : ".").'">'.$Jb[DRIVER].'</a> &raquo; ';
        $_ = substr(preg_replace('~\b(db|ns)=[^&]*&~', '', ME), 0, -1);
        $O = $b->serverName(SERVER);
        $O = ($O != "" ? $O : lang(62));
        if ($Sa === false) echo "$O\n"; else {
            echo "<a href='".($_ ? h($_) : ".")."' accesskey='1' title='Alt+Shift+1'>$O</a> &raquo; ";
            if ($_GET["ns"] != "" || (DB != "" && is_array($Sa))) echo '<a href="'.h($_."&db=".urlencode(DB).(support("scheme") ? "&ns=" : "")).'">'.h(DB).'</a> &raquo; ';
            if (is_array($Sa)) {
                if ($_GET["ns"] != "") echo '<a href="'.h(substr(ME, 0, -1)).'">'.h($_GET["ns"]).'</a> &raquo; ';
                foreach ($Sa
                         as $y => $X) {
                    $Cb = (is_array($X) ? $X[1] : h($X));
                    if ($Cb != "") echo "<a href='".h(ME."$y=").urlencode(is_array($X) ? $X[0] : $X)."'>$Cb</a> &raquo; ";
                }
            }
            echo "$ng\n";
        }
...
get_nonce() {
    static $ge;
    if (!$ge) $ge = base64_encode(rand_string());
    return $ge;
}
...
page_messages($o) {
    ...
    $Wd = $_SESSION["messages"][$Ng];
    if ($Wd) {
        echo "<div class='message'>".implode("</div>\n<div class='message'>", $Wd)."</div>".script("messagesPrint();");
...
function
int32($ce) {
    while ($ce >= 2147483648) $ce -= 4294967296;
    while ($ce <= -2147483649) $ce += 4294967296;
    return (int)$ce;
}
...
function
str2long($qf, $ah) {
    $W = array_values(unpack('V*', str_pad($qf, 4 * ceil(strlen($qf) / 4), "\0")));
    if ($ah) $W[] = strlen($qf);
    return $W;
}

function
xxtea_mx($lh, $kh, $Yf, $sd) {
    return
        int32((($lh >> 5 & 0x7FFFFFF) ^ $kh << 2) + (($kh >> 3 & 0x1FFFFFFF) ^ $lh << 4)) ^ int32(($Yf ^ $kh) + ($sd ^ $lh));
}
...
encrypt_string($Tf, $y) {
    ...
    while ($Ve-- > 0) {
        ...
        for ($_e = 0; $_e < $ce; $_e++) {
            $kh = $W[$_e + 1];
            $be = xxtea_mx($lh, $kh, $Yf, $y[$_e & 3 ^ $Nb]);
            $lh = int32($W[$_e] + $be);
            $W[$_e] = $lh;
        }
        $kh = $W[0];
...
add_invalid_login() {
    global $b;
    $Dc = file_open_lock(get_temp_dir()."/adminer.invalid");
    if (!$Dc) return;
    $nd = unserialize(stream_get_contents($Dc));
    $kg = time();
    if ($nd) {
        foreach ($nd
                 as $od => $X) {
            if ($X[0] < $kg) unset($nd[$od]);
        }
    }
    $md =& $nd[$b->bruteForceKey()];
    if (!$md) $md = [$kg + 30 * 60, 0];
    $md[1]++;
    file_write_unlock($Dc, serialize($nd));
}

function
check_invalid_login() {
    global $b;
    $nd = unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));
    $md = $nd[$b->bruteForceKey()];
    $fe = ($md[1] > 29 ? $md[0] - time() : 0);
    if ($fe > 0) auth_error(lang(64, ceil($fe / 60)));
}
...
if ($Ea) {
    ...
    $O = $Ea["server"];
    $V = $Ea["username"];
    $F = (string)$Ea["password"];
    $m = $Ea["db"];
    set_password($Vg, $O, $V, $F);
    $_SESSION["db"][$Vg][$O][$V][$m] = true;
    if ($Ea["permanent"]) {
        $y = base64_encode($Vg)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($m);
        $Re = $b->permanentLogin(true);
        $Ie[$y] = "$y:".base64_encode($Re ? encrypt_string($F, $Re) : "");
        cookie("adminer_permanent", implode(" ", $Ie));
    }
    if (count($_POST) == 1 || DRIVER != $Vg || SERVER != $O || $_GET["username"] !== $V || DB != $m) redirect(auth_url($Vg, $O, $V, $m));
} elseif ($_POST["logout"]) {
    if ($Oc && !verify_token()) {
        page_header(lang(63), lang(65));
        page_footer("db");
        exit;
    } else {
        foreach (["pwds", "db", "dbs", "queries"] as $y) set_session($y, null);
        unset_permanent();
        redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~', '', ME), 0, -1), lang(66).' '.lang(67));
    }
    ...
    foreach ($Ie
             as $y => $X) {
        [, $ab] = explode(":", $X);
        [$Vg, $O, $V, $m] = array_map('base64_decode', explode("-", $y));
        set_password($Vg, $O, $V, decrypt_string(base64_decode($ab), $Re));
        $_SESSION["db"][$Vg][$O][$V][$m] = true;
    }
}
function
unset_permanent() {
    ...
    foreach ($Ie
             as $y => $X) {
        [$Vg, $O, $V, $m] = array_map('base64_decode', explode("-", $y));
        if ($Vg == DRIVER && $O == SERVER && $V == $_GET["username"] && $m == DB) unset($Ie[$y]);
    }
    cookie("adminer_permanent", implode(" ", $Ie));
}
...
auth_error($o) {
    ...
    if (isset($_GET["username"])) {
        ...
        if (($_COOKIE[$Df] || $_GET[$Df]) && !$Oc) $o = lang(68); else {
            restart_session();
            add_invalid_login();
            $F = get_password();
            if ($F !== null) {
                if ($F === false) $o .= '<br>'.lang(69, target_blank(), '<code>permanentLogin()</code>');
                set_password(DRIVER, SERVER, $_GET["username"], null);
            }
            unset_permanent();
        }
    }
    if (!$_COOKIE[$Df] && $_GET[$Df] && ini_bool("session.use_only_cookies")) $o = lang(70);
    $E = session_get_cookie_params();
    cookie("adminer_key", ($_COOKIE["adminer_key"] ? $_COOKIE["adminer_key"] : rand_string()), $E["lifetime"]);
    page_header(lang(37), $o, null);
    echo "<form action='' method='post'>\n", "<div>";
    if (hidden_fields($_POST, ["auth"])) echo "<p class='message'>".lang(71)."\n";
    echo "</div>\n";
    $b->loginForm();
    echo "</form>\n";
    page_footer("auth");
...
if (isset($_GET["username"]) && !class_exists("Min_DB")) {
    ...
    unset_permanent();
    page_header(lang(72), lang(73, implode(", ", $Me)), false);
    page_footer("auth");
    exit;
}
stop_session(true);
if (isset($_GET["username"]) && is_string(get_password())) {
    [$Uc, $Ke] = explode(":", SERVER, 2);
    if (is_numeric($Ke) && ($Ke < 1024 || $Ke > 65535)) auth_error(lang(74));
    check_invalid_login();
    $h = connect();
    $n = new
    Min_Driver($h);
...
$Jd = null;
if (!is_object($h) || ($Jd = $b->login($_GET["username"], get_password())) !== true) {
    $o = (is_string($h) ? h($h) : (is_string($Jd) ? $Jd : lang(75)));
    auth_error($o.(preg_match('~^ | $~', get_password()) ? '<br>'.lang(76) : ''));
}
if ($Ea && $_POST["token"]) $_POST["token"] = $tg;
$o = '';
if ($_POST) {
    if (!verify_token()) {
        $id = "max_input_vars";
        ...
        $o = (!$_POST["token"] && $Td ? lang(77, "'$id'") : lang(65).' '.lang(78));
    }
} elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
    $o = lang(79, "'post_max_size'");
    if (isset($_GET["sql"])) $o .= ' '.lang(80);
}
function
email_header($Pc) {
    return "=?UTF-8?B?".base64_encode($Pc)."?=";
}

function
send_mail($Rb, $Wf, $Vd, $Ec = "", $pc = []) {
    $Xb = (DIRECTORY_SEPARATOR == "/" ? "\n" : "\r\n");
    $Vd = str_replace("\n", $Xb, wordwrap(str_replace("\r", "", "$Vd\n")));
    $Ra = uniqid("boundary");
    $Ba = "";
    foreach ((array)$pc["error"] as $y => $X) {
        if (!$X) $Ba .= "--$Ra$Xb"."Content-Type: ".str_replace("\n", "", $pc["type"][$y]).$Xb."Content-Disposition: attachment; filename=\"".preg_replace('~["\n]~', '', $pc["name"][$y])."\"$Xb"."Content-Transfer-Encoding: base64$Xb$Xb".chunk_split(base64_encode(file_get_contents($pc["tmp_name"][$y])), 76, $Xb).$Xb;
    }
    $Ma = "";
    $Qc = "Content-Type: text/plain; charset=utf-8$Xb"."Content-Transfer-Encoding: 8bit";
    if ($Ba) {
        $Ba .= "--$Ra--$Xb";
        $Ma = "--$Ra$Xb$Qc$Xb$Xb";
        $Qc = "Content-Type: multipart/mixed; boundary=\"$Ra\"";
    }
    $Qc .= $Xb."MIME-Version: 1.0$Xb"."X-Mailer: Adminer Editor".($Ec ? $Xb."From: ".str_replace("\n", "", $Ec) : "");
    return
        mail($Rb, email_header($Wf), $Ma.$Vd.$Ba, $Qc);
}

function
like_bool($p) {
    return
        preg_match("~bool|(tinyint|bit)\\(1\\)~", $p["full_type"]);
}

$h->select_db($b->database());
$ne = "RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";
$Jb[DRIVER] = lang(37);
...
if (isset($_GET["download"])) {
    ...
    $I = $n->select($a, $M, [where($_GET, $q)], $M);
    $K = ($I ? $I->fetch_row() : []);
    echo $n->value($K[0], $q[$_GET["field"]]);
    exit;
    ...
    foreach ($q
             as $B => $p) {
        if (!isset($p["privileges"][$Mg ? "update" : "insert"]) || $b->fieldName($p) == "" || $p["generated"]) unset($q[$B]);
    }
    if ($_POST && !$o && !isset($_GET["select"])) {
        $Id = $_POST["referer"];
        if ($_POST["insert"]) $Id = ($Mg ? null : $_SERVER["REQUEST_URI"]); elseif (!preg_match('~^.+&select=.+$~', $Id)) $Id = ME."select=".urlencode($a);
        $w = indexes($a);
        $Hg = unique_array($_GET["where"], $w);
        $Xe = "\nWHERE $Z";
        if (isset($_POST["delete"])) queries_redirect($Id, lang(81), $n->delete($a, $Xe, !$Hg)); else {
            $P = [];
            foreach ($q
                     as $B => $p) {
                $X = process_input($p);
                if ($X !== false && $X !== null) $P[idf_escape($B)] = $X;
            }
            if ($Mg) {
                if (!$P) redirect($Id);
                queries_redirect($Id, lang(82), $n->update($a, $P, $Xe, !$Hg));
                if (is_ajax()) {
                    page_headers();
                    page_messages($o);
                    exit;
                ...
                $I = $n->insert($a, $P);
                $Cd = ($I ? last_id() : 0);
                queries_redirect($Id, lang(83, ($Cd ? " $Cd" : "")), $I);
            }
        }
    }
    $K = null;
    if ($_POST["save"]) $K = (array)$_POST["fields"]; elseif ($Z) {
        $M = [];
        foreach ($q
                 as $B => $p) {
            if (isset($p["privileges"]["select"])) {
                $_a = convert_field($p);
                if ($_POST["clone"] && $p["auto_increment"]) $_a = "''";
                if ($x == "sql" && preg_match("~enum|set~", $p["type"])) $_a = "1*".idf_escape($B);
                $M[] = ($_a ? "$_a AS " : "").idf_escape($B);
            }
        }
        $K = [];
        if (!support("table")) $M = ["*"];
        if ($M) {
            $I = $n->select($a, $M, [$Z], $M, [], (isset($_GET["select"]) ? 2 : 1));
            if (!$I) $o = error(); else {
                $K = $I->fetch_assoc();
                if (!$K) $K = false;
            }
    ...
    if (!support("table") && !$q) {
        ...
        if ($K) {
            foreach ($K
                     as $y => $X) {
                if (!$Z) $K[$y] = null;
                $q[$y] = ["field" => $y, "null" => ($y != $n->primary), "auto_increment" => ($y == $n->primary)];
            }
        }
    }
    edit_form($a, $q, $K, $Mg);
    ...
    $w = indexes($a);
    $q = fields($a);
    $Ac = column_foreign_keys($a);
    $me = $S["Oid"];
    parse_str($_COOKIE["adminer_import"], $ta);
    $of = [];
    $f = [];
    $ig = null;
    foreach ($q
             as $y => $p) {
        $B = $b->fieldName($p);
        if (isset($p["privileges"]["select"]) && $B != "") {
            $f[$y] = html_entity_decode(strip_tags($B), ENT_QUOTES);
            if (is_shortable($p)) $ig = $b->selectLengthProcess();
        }
        $of += $p["privileges"];
    }
    [$M, $Hc] = $b->selectColumnsProcess($f, $w);
    $pd = count($Hc) < count($M);
    $Z = $b->selectSearchProcess($q, $w);
    $ve = $b->selectOrderProcess($q, $w);
    $z = $b->selectLimitProcess();
    if ($_GET["val"] && is_ajax()) {
        header("Content-Type: text/plain; charset=utf-8");
        foreach ($_GET["val"] as $Ig => $K) {
            $_a = convert_field($q[key($K)]);
            $M = [$_a ? $_a : idf_escape(key($K))];
            $Z[] = where_check($Ig, $q);
            $J = $n->select($a, $M, $Z, $M);
            if ($J) echo
            reset($J->fetch_row());
    ...
    foreach ($w
             as $v) {
        if ($v["type"] == "PRIMARY") {
            $Oe = array_flip($v["columns"]);
            $Kg = ($M ? $Oe : []);
            foreach ($Kg
                     as $y => $X) {
                if (in_array(idf_escape($y), $M)) unset($Kg[$y]);
            }
            break;
        }
    }
    if ($me && !$Oe) {
        $Oe = $Kg = [$me => 0];
        $w[] = ["type" => "PRIMARY", "columns" => [$me]];
    }
    if ($_POST && !$o) {
        $fh = $Z;
        if (!$_POST["all"] && is_array($_POST["check"])) {
            $Ya = [];
            foreach ($_POST["check"] as $Va) $Ya[] = where_check($Va, $q);
            $fh[] = "((".implode(") OR (", $Ya)."))";
        }
        $fh = ($fh ? "\nWHERE ".implode(" AND ", $fh) : "");
        if ($_POST["export"]) {
            cookie("adminer_import", "output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));
            dump_headers($a);
            $b->dumpTable($a, "");
            $Ec = ($M ? implode(", ", $M) : "*").convert_fields($f, $q, $M)."\nFROM ".table($a);
            $Jc = ($Hc && $pd ? "\nGROUP BY ".implode(", ", $Hc) : "").($ve ? "\nORDER BY ".implode(", ", $ve) : "");
            if (!is_array($_POST["check"]) || $Oe) $G = "SELECT $Ec$fh$Jc"; else {
                $Gg = [];
                foreach ($_POST["check"] as $X) $Gg[] = "(SELECT".limit($Ec, "\nWHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($X, $q).$Jc, 1).")";
                $G = implode(" UNION ALL ", $Gg);
            }
            $b->dumpData($a, "table", $G);
            exit;
        }
        if (!$b->selectEmailProcess($Z, $Ac)) {
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                if (!$_POST["delete"]) {
                    foreach ($f
                             as $B => $X) {
                        $X = process_input($q[$B]);
                        if ($X !== null && ($_POST["clone"] || $X !== false)) $P[idf_escape($B)] = ($X !== false ? $X : idf_escape($B));
                    }
                }
                if ($_POST["delete"] || $P) {
                    if ($_POST["clone"]) $G = "INTO ".table($a)." (".implode(", ", array_keys($P)).")\nSELECT ".implode(", ", $P)."\nFROM ".table($a);
                    if ($_POST["all"] || ($Oe && is_array($_POST["check"])) || $pd) {
                        $I = ($_POST["delete"] ? $n->delete($a, $fh) : ($_POST["clone"] ? queries("INSERT $G$fh") : $n->update($a, $P, $fh)));
                        $ua = $h->affected_rows;
                    } else {
                        foreach ((array)$_POST["check"] as $X) {
                            $bh = "\nWHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($X, $q);
                            $I = ($_POST["delete"] ? $n->delete($a, $bh, 1) : ($_POST["clone"] ? queries("INSERT".limit1($a, $G, $bh)) : $n->update($a, $P, $bh, 1)));
                            if (!$I) break;
                            $ua += $h->affected_rows;
                        }
                    }
                ...
                $Vd = lang(84, $ua);
                if ($_POST["clone"] && $I && $ua == 1) {
                    $Cd = last_id();
                    if ($Cd) $Vd = lang(83, " $Cd");
                }
                queries_redirect(remove_from_uri($_POST["all"] && $_POST["delete"] ? "page" : ""), $Vd, $I);
                if (!$_POST["delete"]) {
                ...
                if (!$_POST["val"]) $o = lang(85); else {
                    $I = true;
                    $ua = 0;
                    foreach ($_POST["val"] as $Ig => $K) {
                        $P = [];
                        foreach ($K
                                 as $y => $X) {
                            $y = bracket_escape($y, 1);
                            $P[idf_escape($y)] = (preg_match('~char|text~', $q[$y]["type"]) || $X != "" ? $b->processInput($q[$y], $X) : "NULL");
                        }
                        $I = $n->update($a, $P, " WHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($Ig, $q), !$pd && !$Oe, " ");
                        if (!$I) break;
                        $ua += $h->affected_rows;
                    }
                    queries_redirect(remove_from_uri(), lang(84, $ua), $I);
                }
            } elseif (!is_string($oc = get_file("csv_file", true))) $o = upload_error($oc);
			elseif (!preg_match('~~u', $oc)) $o = lang(86);
            else {
                cookie("adminer_import", "output=".urlencode($ta["output"])."&format=".urlencode($_POST["separator"]));
                $I = true;
                $hb = array_keys($q);
                preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~', $oc, $Pd);
                $ua = count($Pd[0]);
                $n->begin();
                $N = ($_POST["separator"] == "csv" ? "," : ($_POST["separator"] == "tsv" ? "\t" : ";"));
                $L = [];
                foreach ($Pd[0] as $y => $X) {
                    preg_match_all("~((?>\"[^\"]*\")+|[^$N]*)$N~", $X.$N, $Qd);
                    if (!$y && !array_diff($Qd[1], $hb)) {
                        $hb = $Qd[1];
                        $ua--;
                    } else {
                        $P = [];
                        foreach ($Qd[1] as $s => $eb) $P[idf_escape($hb[$s])] = ($eb == "" && $q[$hb[$s]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $eb))));
                        $L[] = $P;
                    }
                }
                $I = (!$L || $n->insertUpdate($a, $L, $Oe));
                if ($I) $I = $n->commit();
                queries_redirect(remove_from_uri("page"), lang(87, $ua), $I);
                $n->rollback();
            }
        }
    }
    ...
    if (is_ajax()) {
        ...
        page_header(lang(49).": $bg", $o);
    $P = null;
    if (isset($of["insert"]) || !support("table")) {
        $P = "";
        foreach ((array)$_GET["where"] as $X) {
            if ($Ac[$X["col"]] && count($Ac[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~', $X["val"])))) $P .= "&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);
        }
    }
    $b->selectLinks($S, $P);
    if (!$f && support("table")) echo "<p class='error'>".lang(88).($q ? "." : ": ".error())."\n"; else {
        ...
        $b->selectOrderPrint($ve, $f, $w);
        $b->selectLimitPrint($z);
        $b->selectLengthPrint($ig);
        $b->selectActionPrint($w);
        echo "</form>\n";
        $D = $_GET["page"];
        if ($D == "last") {
            $Cc = $h->result(count_rows($a, $Z, $pd, $Hc));
            $D = floor(max(0, $Cc - 1) / $z);
        }
        $vf = $M;
        $Ic = $Hc;
        if (!$vf) {
            $vf[] = "*";
            $qb = convert_fields($f, $q, $M);
            if ($qb) $vf[] = substr($qb, 2);
        }
        foreach ($M
                 as $y => $X) {
            $p = $q[idf_unescape($X)];
            if ($p && ($_a = convert_field($p))) $vf[$y] = "$_a AS $X";
        }
        if (!$pd && $Kg) {
            foreach ($Kg
                     as $y => $X) {
                $vf[] = idf_escape($y);
                if ($Ic) $Ic[] = idf_escape($y);
            }
        }
        $I = $n->select($a, $vf, $Z, $Ic, $ve, $z, $D, true);
        if (!$I) echo "<p class='error'>".error()."\n"; else {
            if ($x == "mssql" && $D) $I->seek($z * $D);
            $Tb = [];
            echo "<form action='' method='post' enctype='multipart/form-data'>\n";
            $L = [];
            while ($K = $I->fetch_assoc()) {
                if ($D && $x == "oracle") unset($K["RNUM"]);
                $L[] = $K;
            }
            if ($_GET["page"] != "last" && $z != "" && $Hc && $pd && $x == "sql") $Cc = $h->result(" SELECT FOUND_ROWS()");
            if (!$L) echo "<p class='message'>".lang(12)."\n"; else {
                $La = $b->backwardKeys($a, $bg);
                echo "<div class='scrollable'>", "<table id='table' cellspacing='0' class='nowrap checkable'>", script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"), "<thead><tr>".(!$Hc && $M ? "" : "<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);", "")." <a href='".h($_GET["modify"] ? remove_from_uri("modify") : $_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(89)."</a>");
                $de = [];
                $Gc = [];
                reset($M);
                $Ze = 1;
                foreach ($L[0] as $y => $X) {
                    if (!isset($Kg[$y])) {
                        ...
                        $p = $q[$M ? ($X ? $X["col"] : current($M)) : $y];
                        $B = ($p ? $b->fieldName($p, $Ze) : ($X["fun"] ? "*" : $y));
                        if ($B != "") {
                            $Ze++;
                            $de[$y] = $B;
                            $e = idf_escape($y);
                            $Vc = remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);
                            $Cb = "&desc%5B0%5D=1";
                            echo "<th>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});", ""), '<a href="'.h($Vc.($ve[0] == $e || $ve[0] == $y || (!$ve && $pd && $Hc[0] == $e) ? $Cb : '')).'">';
                            echo
                                apply_sql_function($X["fun"], $B)."</a>";
                            echo "<span class='column hidden'>", "<a href='".h($Vc.$Cb)."' title='".lang(90)."' class='text'> ↓</a>";
                            if (!$X["fun"]) {
                                echo '<a href="#fieldset-search" title="'.lang(44).'" class="text jsonly"> =</a>', script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");
                            }
                            echo "</span>";
                        }
                        $Gc[$y] = $X["fun"];
                ...
                if ($_GET["modify"]) {
                    foreach ($L
                             as $K) {
                        foreach ($K
                                 as $y => $X) $Fd[$y] = max($Fd[$y], min(40, strlen(utf8_decode($X))));
                    }
                }
                echo ($La ? "<th>".lang(91) : "")."</thead>\n";
                if (is_ajax()) {
                    if ($z % 2 == 1 && $D % 2 == 1) odd();
                    ob_end_clean();
                }
                foreach ($b->rowDescriptions($L, $Ac) as $ce => $K) {
                    $Hg = unique_array($L[$ce], $w);
                    if (!$Hg) {
                        $Hg = [];
                        foreach ($L[$ce] as $y => $X) {
                            if (!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~', $y)) $Hg[$y] = $X;
                        }
                    }
                    $Ig = "";
                    foreach ($Hg
                             as $y => $X) {
                        if (($x == "sql" || $x == "pgsql") && preg_match('~char|text|enum|set~', $q[$y]["type"]) && strlen($X) > 64) {
                            $y = (strpos($y, '(') ? $y : idf_escape($y));
                            $y = "MD5(".($x != 'sql' || preg_match("~^utf8~", $q[$y]["collation"]) ? $y : "CONVERT($y USING ".charset($h).")").")";
                            $X = md5($X);
                        }
                        $Ig .= "&".($X !== null ? urlencode("where[".bracket_escape($y)."]")."=".urlencode($X) : "null%5B%5D=".urlencode($y));
                    }
                    echo "<tr".odd().">".(!$Hc && $M ? "" : "<td>".checkbox("check[]", substr($Ig, 1), in_array(substr($Ig, 1), (array)$_POST["check"])).($pd || information_schema(DB) ? "" : " <a href='".h(ME."edit=".urlencode($a).$Ig)."' class='edit'>".lang(92)."</a>"));
                    foreach ($K
                             as $y => $X) {
                        if (isset($de[$y])) {
                            $p = $q[$y];
                            $X = $n->value($X, $p);
                            if ($X != "" && (!isset($Tb[$y]) || $Tb[$y] != "")) $Tb[$y] = (is_mail($X) ? $de[$y] : "");
                            $_ = "";
                            if (preg_match('~blob|bytea|raw|file~', $p["type"]) && $X != "") $_ = ME.'download='.urlencode($a).'&field='.urlencode($y).$Ig;
                            if (!$_ && $X !== null) {
                                foreach ((array)$Ac[$y] as $_c) {
                                    if (count($Ac[$y]) == 1 || end($_c["source"]) == $y) {
                                        $_ = "";
                                        foreach ($_c["source"] as $s => $Kf) $_ .= where_link($s, $_c["target"][$s], $L[$ce][$Kf]);
                                        $_ = ($_c["db"] != "" ? preg_replace('~([?&]db=)[^&]+~', '\1'.urlencode($_c["db"]), ME) : ME).'select='.urlencode($_c["table"]).$_;
                                        if ($_c["ns"]) $_ = preg_replace('~([?&]ns=)[^&]+~', '\1'.urlencode($_c["ns"]), $_);
                                        if (count($_c["source"]) == 1) break;
                                    }
                                }
                            }
                            if ($y == "COUNT(*)") {
                                ...
                                foreach ((array)$_GET["where"] as $W) {
                                    if (!array_key_exists($W["col"], $Hg)) $_ .= where_link($s++, $W["col"], $W["val"], $W["op"]);
                                }
                                foreach ($Hg
                                         as $sd => $W) $_ .= where_link($s++, $sd, $W);
                            }
                            $X = select_value($X, $_, $p, $ig);
                            $t = h("val[$Ig][".bracket_escape($y)."]");
                            $Y = $_POST["val"][$Ig][bracket_escape($y)];
                            $Pb = !is_array($K[$y]) && is_utf8($X) && $L[$ce][$y] == $K[$y] && !$Gc[$y];
                            $hg = preg_match('~text|lob~', $p["type"]);
                            echo "<td id='$t'";
                            if (($_GET["modify"] && $Pb) || $Y !== null) {
                                $Lc = h($Y !== null ? $Y : $K[$y]);
                                echo ">".($hg ? "<textarea name='$t' cols='30' rows='".(substr_count($K[$y], "\n") + 1)."'>$Lc</textarea>" : "<input name='$t' value='$Lc' size='$Fd[$y]'>");
                            } else {
                                $Kd = strpos($X, "<i>…</i>");
                                echo " data-text='".($Kd ? 2 : ($hg ? 1 : 0))."'".($Pb ? "" : " data-warning='".h(lang(93))."'").">$X</td>";
                            }
                        }
                    }
                    if ($La) echo "<td>";
                    $b->backwardKeysPrint($La, $L[$ce]);
                    echo "</tr>\n";
                }
                if (is_ajax()) exit;
                echo "</table>\n", "</div>\n";
            ...
            if (!is_ajax()) {
                if ($L || $D) {
                    $bc = true;
                    if ($_GET["page"] != "last") {
                        if ($z == "" || (count($L) < $z && ($L || !$D))) $Cc = ($D ? $D * $z : 0) + count($L); elseif ($x != "sql" || !$pd) {
                            $Cc = ($pd ? false : found_rows($S, $Z));
                            if ($Cc < max(1e4, 2 * ($D + 1) * $z)) $Cc = reset(slow_query(count_rows($a, $Z, $pd, $Hc))); else$bc = false;
                        }
                    }
                    $Ae = ($z != "" && ($Cc === false || $Cc > $z || $D));
                    if ($Ae) {
                        echo(($Cc === false ? count($L) + 1 : $Cc - $D * $z) > $z ? '<p><a href="'.h(remove_from_uri("page")."&page=".($D + 1)).'" class="loadmore">'.lang(94).'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".lang(95)."…');", "") : ''), "\n";
                    }
                }
                echo "<div class='footer'><div>\n";
                if ($L || $D) {
                    if ($Ae) {
                        $Rd = ($Cc === false ? $D + (count($L) >= $z ? 2 : 1) : floor(($Cc - 1) / $z));
                        echo "<fieldset>";
                        if ($x != "simpledb") {
                            echo "<legend><a href='".h(remove_from_uri("page"))."'>".lang(96)."</a></legend>", script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".lang(96)."', '".($D + 1)."')); return false; };"), pagination(0, $D).($D > 5 ? " …" : "");
                            for ($s = max(1, $D - 4); $s < min($Rd, $D + 5); $s++) echo
                            pagination($s, $D);
                            if ($Rd > 0) {
                                echo($D + 5 < $Rd ? " …" : ""), ($bc && $Cc !== false ? pagination($Rd, $D) : " <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Rd'>".lang(97)."</a>");
                            }
                        } else {
                            echo "<legend>".lang(96)."</legend>", pagination(0, $D).($D > 1 ? " …" : ""), ($D ? pagination($D, $D) : ""), ($Rd > $D ? pagination($D + 1, $D).($Rd > $D + 1 ? " …" : "") : "");
                        }
                        echo "</fieldset>\n";
                    }
                    echo "<fieldset>", "<legend>".lang(98)."</legend>";
                    $Hb = ($bc ? "" : "~ ").$Cc;
                    echo
                    ...
                    if ($b->selectCommandPrint()) {
                        echo '<fieldset', ($_GET["modify"] ? '' : ' class="jsonly"'), '><legend>', lang(89), '</legend><div>
...
';
                    ...
                    if ($Bc) {
                        print_fieldset("export", lang(102)." <span id='selected2'></span>");
                        $ze = $b->dumpOutput();
                        echo($ze ? html_select("output", $ze, $ta["output"])." " : ""), html_select("format", $Bc, $ta["format"]), " <input type='submit' name='export' value='".lang(102)."'>\n", "</div></fieldset>\n";
                    }
                    $b->selectEmailPrint(array_filter($Tb, 'strlen'), $f);
                }
                echo "</div></div>\n";
                if ($b->selectImportPrint()) {
                    echo "<div>", "<a href='#import'>".lang(103)."</a>", script("qsl('a').onclick = partial(toggle, 'import');", ""), "<span id='import' class='hidden'>: ", "<input type='file' name='csv_file'> ", html_select("separator", ["csv" => "CSV,", "csv;" => "CSV;", "tsv" => "TSV"], $ta["format"], 1);
                    echo " <input type='submit' name='import' value='".lang(103)."'>", "</span>", "</div>";
                }
                echo "<input type='hidden' name='token' value='$tg'>\n", "</form>\n", (!$Hc && $M ? "" : script("tableCheck();"));
            }
        }
    }
    if (is_ajax()) {
    ...
    if ($_GET["script"] == "kill") $h->query("KILL ".number($_POST["kill"])); elseif (list($R, $t, $B) = $b->_foreignColumn(column_foreign_keys($_GET["source"]), $_GET["field"])) {
        $z = 11;
        $I = $h->query("SELECT $t, $B FROM ".table($R)." WHERE ".(preg_match('~^[0-9]+$~', $_GET["value"]) ? "$t = $_GET[value] OR " : "")."$B LIKE ".q("$_GET[value]%")." ORDER BY 2 LIMIT $z");
        for ($s = 1; ($K = $I->fetch_row()) && $s < $z; $s++) echo "<a href='".h(ME."edit=".urlencode($R)."&where".urlencode("[".bracket_escape(idf_unescape($t))."]")."=".urlencode($K[0]))."'>".h($K[1])."</a><br>\n";
        if ($K) echo "...\n";
    }
    exit;
} else {
    page_header(lang(62), "", false);
    if ($b->homepage()) {
        ...
        foreach (table_status() as $R => $K) {
            $B = $b->tableName($K);
            if (isset($K["Engine"]) && $B != "") {
                echo '<tr'.odd().'><td>'.checkbox("tables[]", $R, in_array($R, (array)$_POST["tables"], true)), "<th><a href='".h(ME).'select='.urlencode($R)."'>$B</a>";
                $X = format_number($K["Rows"]);
                echo "<td align='right'><a href='".h(ME."edit=").urlencode($R)."'>".($K["Engine"] == "InnoDB" && $X ? "~ $X" : $X)."</a>";
            }
        }
        echo "</table>\n", "</div>\n", "</form>\n", script("tableCheck();");
    }
